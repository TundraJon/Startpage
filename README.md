...
- [ ] Weather options menu — "Units & Core Info" section: reorder rows to Temperature, Wind speed (unit), Wind Speed, Visibility, Cloud Cover %, Feels Like, UV Index (currently Temperature, Wind speed, Feels Like, UV Index, Wind Speed display, Visibility display, Cloud Cover % display). Also drop the word "display" from the three toggle labels — "Wind Speed display" → "Wind Speed", "Visibility display" → "Visibility", "Cloud Cover % display" → "Cloud Cover %" — since they're already toggles, "display" is redundant.
