...
- [ ] **Requested changes, confirmed with the user:**
  ...
  3. `HAIL_GRAVITY` becomes a tunable, not a fixed constant: convert it into a Testing Panel slider ranging **0.1–3** (revised down from an initial 0.5–5 range — the user determined 3, the pre-revamp value, was already too strong, so 3 becomes the slider's ceiling rather than its middle), defaulting to **1.5** (the value confirmed above), with reset support consistent with the other cloud/fog tunables. This came out of a discussion clarifying that `HAIL_GRAVITY`'s magnitude was always an empirically-chosen stylistic value (px/frame², not tied to any real-world unit conversion) rather than a physically-derived constant — the *shape* of the motion (true parabola, trig decomposition) is what's physics-accurate, not the specific number — so it makes more sense as an adjustable knob like the other hail/cloud/fog constants than a hardcoded value.
- [ ] **Expected/intended consequences of this combination, worth confirming look right once built:** since bounce height is `vy0²/(2g)` and flight time is `2vy0/g`, halving gravity (from 3 to the new 1.5 default) alone roughly doubles both height and duration for a given launch speed; the 25% speed increase adds further height/duration on top of that — together this should produce the higher bounces the user wants. Separately, shrinking the boost multiplier from 2× to 1.5× directly narrows the height gap between boosted and unboosted stones from a 4× ratio (2²) down to a 2.25× ratio (1.5²), which is the specific lever for "not such a massive difference in bounce heights." The user also described wanting "smooth falling down, without a long trail" as the overall goal of this pass — noted here as intent/context for whoever builds it, not a separate mechanism to design; the numeric changes above (plus the new gravity slider) are the full extent of what's being requested.

### Floating clouds visually stack/cluster at high cloud cover — add a respawn hold delay

- [ ] **Diagnosis:** this is a different bug from the one addressed by the earlier "cloud respawn jump" / "cloud tab-visibility" fixes (script.js `spawnCloud`/`randomizeCloud`) — those fixed a single cloud's own animation glitching on respawn (replacing the element instead of mutating a running animation in place). The actual issue here: each cloud gets fully independent random `top` (0-50%), size, and `animation-duration` (17-60s, faster/bigger near the top of the band) with no coordination between different cloud elements at all. On respawn (`spawnCloud(oldCloud)`, triggered by `animationiteration`), the fresh replacement gets **no delay whatsoever** — it starts drifting from the left edge (`.wx-skin-cloud`'s `from { left: calc(-1 * var(--cloud-w)) }` keyframe) immediately. With no per-cloud entry-timing variation, different clouds' respawns can stay in a persistent, repeating phase relationship — a faster (bigger, top-band) cloud re-enters at a rate that keeps realigning with a slower one, producing the recurring stacked/curved cluster the user is seeing at high cloud counts (confirmed via screenshot at 100% test cloud cover). The initial page-load batch already gets a *different*, working stagger — a negative `animation-delay` (`-Math.random() * duration`) assigned in `renderWeatherSkin()` right after `spawnCloud()` — so that part is untouched.
- [ ] **Requested fix, proposed by the user:** give each cloud a random "hold" delay of 0.1–5 seconds before it starts drifting, applied specifically on respawn (inside `spawnCloud`, only when `oldCloud` is passed — i.e. not the initial batch, which keeps its existing negative-delay stagger). Concretely: `if (oldCloud) cloud.style.animationDelay = (0.1 + Math.random() * 4.9) + 's';` after `randomizeCloud(cloud)` is called. With no `animation-fill-mode` set, the cloud sits at its base (off-canvas, `left: calc(-1 * var(--cloud-w))`) during the hold, matching "off the left side of the viewable canvas" exactly.
- [ ] **Why this should work:** re-rolling a random delay on every single respawn makes each cloud's effective loop period `duration + random delay` instead of a fixed `duration`, so no two clouds can stay phase-locked and keep re-entering together — breaking up the *persistent* clustering that's the actual complaint. Not an absolute structural guarantee against any momentary overlap (vertical lane position is still independently random per cloud), but should eliminate the recurring stacked look.

### Live Weather diagnostics panel (temporary): moon phase stuck on "Full Moon" for 5+ days

- [ ] **Root cause suspected but not yet confirmed:** `weatherState`'s placeholder default (script.js:461) hardcodes `moonPhase: 'Full Moon'` before any live data loads. The only place it's ever overwritten is `if (astro.moon_phase) weatherState.moonPhase = astro.moon_phase;` (script.js:1644) — a silent no-op if the field is ever missing/falsy in the real API response, with no error surfaced anywhere. User has no dev tools access on mobile Chrome (Pixel 7) to inspect `localStorage`/network directly, so a temporary on-page diagnostic display is needed to actually see what the live API is returning.
- [ ] **Requested addition:** a new "Live Weather Diagnostics" section in the Testing Panel (clearly marked temporary, same spirit as the panel's existing "not part of the real app" note), read-only text, showing:
  - **Before the call:** the coordinates used for the last fetch (confirms geolocation isn't stuck on the `FALLBACK_COORDS` Albuquerque default); whether a key is present, masked (first 4 / last 4 characters shown, middle starred out — never full plaintext, consistent with the standing rule that the real key must never be exposed in the repo, and shouldn't be needlessly exposed on-screen either even though this is runtime-only and never committed); cache state — is there a cached response, its age in minutes, and whether `loadLiveWeather` (script.js:1675) considers it stale.
  - **Outcome of the call:** whether the last load fetched fresh data, served from cache because it wasn't stale, or fell back to stale cache after a fetch error (the existing `catch (e) { console.error(...); if (cache) applyLiveWeatherData(cache.data); }` path, script.js:1695-1697) — and if it errored, the actual caught error message, currently swallowed by `console.error` with nothing surfaced anywhere visible on mobile.
  - **The full raw response:** the complete `astro` object as last received (sunrise, sunset, moon_phase, moon_illumination if present) to directly answer the moon-phase question; the `condition` object (code + text) for cross-checking against `WX_CONDITIONS`; and realistically the entire raw JSON response, pretty-printed in a scrollable read-only block, so anything else latent surfaces too, not just what we think to check for right now.
- [ ] **Also requested: a "Copy diagnostics" button.** The user is on mobile with no dev tools, so hand-transcribing a big JSON blob to relay back isn't practical — needs one tap to copy everything (masked-key state, coords, cache age/staleness, fetch outcome/error message, and the full raw response) as one plain-text blob via the Clipboard API (`navigator.clipboard.writeText`), so it can be pasted directly into a message. Needs a fallback for browsers/contexts where the Clipboard API is unavailable or denied (e.g. a selectable `<textarea>` with the same content, so a manual long-press-select-all-copy still works).
- [ ] **Scope note:** this is explicitly a temporary diagnostic tool for tracking down the moon-phase bug (and any other silent live-data issues it happens to reveal) — intended to be removed once the underlying bug is found and fixed, not a permanent addition to the Testing Panel.
