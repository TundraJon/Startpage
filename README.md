...
- [x] **Dropdown row content — decided by the user, three distinct separators:** `>` between real category-chain levels, `_` before a grouping name (a Visual Grouping Header divider the tile sits under, if any), and `~` before the tile name itself — mnemonic: tilde ~ tile, per the user. E.g. `Games > Computer > Role Play _ Free ~ Wolfenstein`; a tile with no grouping would read `Games > Computer > Role Play ~ Wolfenstein`. Content and all three separators are settled.
