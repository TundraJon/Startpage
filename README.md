...
- [ ] **Conflict confirmed — header height mismatch:** measured directly, Home's header renders at **38px** tall vs. standard headers at **42px** — the standard header's `border-top: 3px solid ...` (styles.css:181) adds 3-4px to the box's total rendered height on top of matching padding (`10px 14px` on both), which directly violates spec Section 3: "header bar height... shouldn't differ just because of the color treatment."
- [ ] **Conflict confirmed — unassigned stripe still shows a gray stripe:** `border-top: 3px solid var(--stripe-color, var(--fg-muted))` (styles.css:181) falls back to `var(--fg-muted)` (a visible gray, confirmed via computed style `rgb(107,107,107)`) when a category has no `--stripe-color` set. Spec Section 1 requires an unassigned category to show **no stripe at all** (neutral header only).
- [ ] **Requested fix (resolves both conflicts at once):** replace the border-based stripe with an inset box-shadow, which doesn't add to box height and can cleanly default to transparent: change styles.css:180-181 from `border: none; border-top: 3px solid var(--stripe-color, var(--fg-muted));` to `border: none; box-shadow: inset 0 3px 0 0 var(--stripe-color, transparent);`. Verified this reference approach in isolation would equalize both headers to the same height and show nothing for an unassigned category.
