...
- [ ] **Not explicitly addressed, my own inference only — flagging clearly rather than assuming silently:**
  - Long-pressing *empty* grid space (today creates a new grouping inline, in the grid) — does this also now route to opening Edit Category (e.g., opened with nothing selected, ready to use the dialog's own ➕), or does empty-space long-press stay as a grid-level creation shortcut since the user's complaint was specifically about *editing* existing groupings, not creating new ones? Genuinely unclear either way.
  - Home's global "+" menu's "+ Grouping" option — same question: still inserts directly in the grid, or now opens Edit Category too?
  - ✏️ inside the dialog's shared icon row — presumed (not confirmed) to turn that list row's name into an inline-editable field right there in the dialog, continuing the established "no popup" philosophy from the grid redesign it replaces, rather than some other mechanism.
