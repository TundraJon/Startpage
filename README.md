...
- [ ] **Open, not yet decided — what ➕ actually does:** does it insert a new divider (right after the selected one) and drop straight into that same inline-rename state with an empty label ready to type, keeping the whole flow popup-free? Or does it still need some minimal prompt for the name? The user's framing ("gives the whole suite of utility right there") reads like the former — recommending zero-dialog inline-rename-on-insert to match — but not confirmed.
- [ ] **Open, not yet decided — what happens to the two existing entry points from Build 58:** long-pressing *empty* grid space (today opens the "New Grouping" popup dialog) and the Groupings list inside Edit Category (today's ✏️ opens the same popup). Does this redesign fully replace both of those too, or do they stay as slower secondary paths alongside the new long-press-the-divider-itself flow?
