...
### Create UI: a "+" entry point for both new Tiles and new Categories, at the current location
...
- [ ] **Placement — a persistent "+" in the pinned header, not per-category:** the doc asked for a "prominent, always-accessible" control. Home's own header has a small action area (`.home-header-actions`, next to the collapse-all button) but Home itself scrolls out of view like any other section — not actually always-accessible. `.pinned-header`'s `.header-icons` (next to Help "?" and Profile, `index.html`) is the one part of the page that's genuinely always on screen regardless of scroll position, so the new "+" button goes there.
