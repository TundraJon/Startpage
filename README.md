...
### Weather widget: Hourly Forecast slide-down panel

- [ ] **Source:** user-supplied spec doc ("Weather Widget — Hourly Forecast Slide-Down Panel"), reproduced/summarized below with exact code references checked against the current implementation.
- [ ] **Precedence check, per the doc's own rule — no actual conflicts found:**
  - The doc worried about reintroducing a "night only" moon phase restriction. Confirmed in code (index.html:110/527, script.js): moon phase display is a simple always-on/off toggle (`data-toggle-setting="moonPhase"`), no day/night gating anywhere. Nothing to conflict with — the doc's precedence warning is moot here.
  - The trigger this doc specifies **already exists as a placeholder**: `weatherEmojiBtn` (script.js:716, 738-740) currently opens a "coming soon" modal (`showComingSoon('Hourly forecast', ...)`) on click — this build replaces that placeholder with the real panel, same trigger element.
  - The "push, don't overlay" category-accordion precedent the doc cites works via a plain `hidden` attribute toggle on an in-flow block (script.js:142-144) — instant, no slide animation. The new panel needs an actual slide transition (per the doc), so it needs its own open/close animation (e.g. a `max-height`/`grid-template-rows` transition), not a literal reuse of the category mechanism — noting this so whoever builds it doesn't expect an existing animated-panel helper to copy from.
- [ ] **Data source — one gap found needing a small fetch change:** `weatherState.hourly` (script.js:1947, `fday.hour || []`) is already populated from the existing single `forecast.json` call and completely unused elsewhere — confirmed no redundant call is needed, matching the doc's requirement. However, the current fetch URL (script.js:1991) requests `days=1`, which returns only *today's* 24 hourly entries (00:00-23:00) — late in the day (e.g. 8pm), fewer than 12 hours of *future* data would remain in that array, unable to satisfy "next 12 hours from now" at all times of day. **Resolution:** bump the URL's `days=1` to `days=2` (still one single call, no new endpoint or duplicate request — same spirit as the doc's "no additional API call" requirement) and concatenate `forecastday[0].hour` + `forecastday[1].hour` before slicing the 12-hour window starting at the current hour.
- [ ] **Trigger & dismissal:** tap `weatherEmojiBtn` toggles the panel open/closed (replacing the coming-soon modal). Dismiss via: tap the emoji again, tap anywhere outside the panel (document-level click listener while open, closing and removing itself on dismiss), or a vertical swipe-up gesture on the panel itself (pointerdown/move/up tracking vertical delta, similar in spirit to the existing `attachLongPress` pointer-event pattern, script.js:154+, though this is a distinct gesture — a swipe, not a hold).
- [ ] **Panel structure:** new element inserted directly after `#weather-widget` (index.html:77) in normal document flow (not `position: absolute`), so its height genuinely pushes later page content down when expanded, matching "push don't overlay." Width matches the weather widget's own width.
- [ ] **Per-hour column (12 total, current hour first/leftmost on open):** built from `weatherState.hourly`, sliced to 12 entries starting at the current hour (with the `days=2` concatenation above ensuring 12 always exist). Each column: time label (parse WeatherAPI's `hour[].time`, `"YYYY-MM-DD HH:MM"`, to a 12-hour "3PM" style label), condition emoji via the existing `weatherIconForCode(hour.condition.code)`, temperature via `hour.temp_f`/`hour.temp_c` through the existing unit-conversion/display path, and precip % (`hour.chance_of_rain`/`hour.chance_of_snow` — take the max of the two, consistent with how precip type is otherwise inferred elsewhere) shown only when greater than 0%, per the existing "omit irrelevant zero data" pattern. Current-hour column gets a distinct tint/border.
- [ ] **Precipitation intensity graph:** a thin SVG polyline/graph beneath the hourly row, plotting the same max(chance_of_rain, chance_of_snow) values across the 12-hour span as a continuous curve. Renders flat/minimal (not omitted, per the doc, so the panel's layout doesn't shift) when all 12 hours are 0%.
- [ ] **Scrolling:** horizontal-only (`overflow-x: auto`, native momentum scrolling — no custom drag-scroll JS needed for this part), sized so 4-5 hour columns are visible at once on a typical phone width, opened pre-scrolled so the current-hour column starts at the left edge.
