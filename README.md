...
- [ ] **Promoted from the Build Planner's "Organizational system overhaul" entry, item 3.** Right now a tile can be added through the UI (each grid's own `+` button), but there's no way to add a new *category* at all, even though Build 47's category data model fully supports it. This entry builds the entry point for both.
- [ ] **Placement — corrected by the user: on Home's own header, not the pinned header.** Originally proposed putting "+" in `.pinned-header`'s `.header-icons` instead, reasoning Home's header wasn't actually always-visible (it scrolls away like any other section) — the user's follow-up resolves that directly instead of routing around it (see the next bullet: Home's header becomes sticky). "+" goes inside `.home-header-actions` (`index.html`), to the *left* of the existing collapse-all (▲) button — so the row reads `+  ▲` — matching the user's own original toolbar sketch from before the document superseded it.
- [ ] **New prerequisite: Home's category header must become sticky, anchored just below the clock/weather widgets — not the rest of Home's tiles, just the header bar.** Currently `.category-header--home` is in normal flow, right after `.pinned-header` (which is itself `position: sticky; top: 0`, variable height — clock/weather widgets, hourly panel, and the alert ticker toggling visibility all change it). Home's tile-grid stays in normal flow and scrolls away as today; only the header bar pins.
  - **Proposed approach:** make `.category-header--home` `position: sticky` with `top` driven by a CSS custom property (`--pinned-header-height`) rather than a hardcoded pixel value, since `.pinned-header`'s real height changes (alert ticker, hourly forecast content, window resize). A `ResizeObserver` on `.pinned-header` (script.js) keeps that custom property in sync automatically on any size change, without needing to hook every individual place that could change its height.
  - `z-index` needs to sit below `.pinned-header`'s (100) so the pinned-header stays on top when both are visible, but above ordinary scrolling content beneath it (e.g. `90`) so Home's header visually stays above whatever scrolls underneath. Home's header already has an opaque background (`var(--home-header-bg)`), so no separate change needed there.
- [ ] **Tapping "+" opens a small two-option menu — "+ Tile" and "+ Category"** — reusing the same small-popup pattern already used everywhere else (tile menu, clock options, etc.: a `.help-overlay`/`.options-panel`), not a new UI pattern.
- [ ] **"Current location" — decided, not left open:** the deepest entry in `openPath`, defaulting to `'home'` when `openPath` is empty (nothing currently navigated into).
- [ ] **"+ Tile" reuses the existing add-tile flow as-is** — `openAddTile(grid, categoryId)` already does everything needed (name/URL overlay, validation, favicon auto-fetch is automatic via the existing Google favicon service). This just resolves `grid`/`categoryId` from "current location" (via `categoryGrids.get(destId)`) instead of requiring a tap on that specific category's own `+` button.
- [ ] **"+ Category" is genuinely new:** a small name-only overlay (matching item 4's own scope — no color picker here; new categories get `stripeColor: null` for now, picking a color is entirely item 4's job, still in the Planner). On submit:
  - Validate + conflict-check: no two sibling categories (same `parentId`) may share a name (case-insensitive compare), matching the doc's ask — reject with an inline message rather than silently allowing a duplicate.
  - Generate a new id the same way tiles do (`newCategoryId()`, mirroring `newTileId()`'s `crypto.randomUUID()`-with-fallback pattern), write a new entry into `categoryTree` (`name`, `parentId` = current location's id (`null` if current location is Home), `order` = one past the current max among its new siblings, `stripeColor: null`, `createdAt`), and `saveCategoryTree`.
- [ ] **The trickiest real edge case: adding the first subcategory under a category that's currently flat.** A flat category's DOM is a plain `.tile-grid` directly under `.category`; gaining a subcategory means it needs to become a `.category-content` wrapper (own-grid + nested section) instead — `buildCategorySection` (script.js) already renders either shape correctly based on whether a category has children, but that only happens once, at initial page load.
- [ ] **Proposed approach — full re-render on any category creation, not surgical DOM patching:** rather than hand-writing DOM surgery for the flat→nested transition (and keeping `categoryToggles`/`categoryGrids` in sync with it), refactor the three blocks that currently only run once at load — `renderCategoryTree()`, the category-header-wiring `forEach`, and the tile-grid-rendering `forEach` — so they're callable again as one `rebuildCategoriesAndTiles()`-style function: clear everything under `#categories` except Home, then re-run all three in sequence against the updated `categoryTree`. This is simple and correct (no special-casing the flat-to-nested transition at all) at the cost of a full rebuild instead of a targeted insert — trivial overhead for a personal homepage's category count. `openPath` is already persisted and `renderOpenPath()` already restores purely from that state (not from DOM), so calling it again after the rebuild correctly re-shows whatever was open before.
...
- [x] **3. Create UI — promoted to the Build Queue** (see "Create UI: a '+' entry point for both new Tiles and new Categories" above).
...
- [ ] **10. Navigation polish: breadcrumbs, up-a-level.** The app already has clear nesting (indentation via `--depth`) and expand/collapse (single-open-path system, `openPath`/`renderOpenPath`), but no breadcrumb trail or explicit "go up one level" control — today, going up means tapping a higher-level category's own collapse button (per the recent per-ancestor collapse button build) rather than a dedicated breadcrumb UI. Worth deciding whether a breadcrumb strip adds real value on top of that, or is redundant with it.
...
- [ ] **Open, not yet decided:** exact shape of "current location" for create (item 3) when nothing is open; whether category color extends to subcategories at all (item 4); the undo stack's design (item 12); soft-delete/trash's design (item 9); whether breadcrumbs are worth adding given per-ancestor collapse buttons already provide a "go up" path (item 10). None of these block writing the category data model (item 1) first, which is the one everything else is downstream of.
