...
### Weather Widget Phase 2 (from `weather-widget-phase2-spec-v2-CORRECTED`)

Resolved conflict: the spec's stacked wind/visibility/cloud layout **overrides** the current horizontal build (confirmed with user 2026-08-20). Feels-like cold emoji (🌬️) already matches the spec — no change needed there.

- [ ] Top-left zone (wind/visibility/cloud): switch from horizontal (icon+value side by side) to stacked — emoji on top, value below, three independent mini-columns. Each independently toggleable; if off, its stack is simply omitted.
- [ ] Feels-like: only display when it differs from actual temp by ±3°F or more (currently always shown regardless of magnitude, in both directions).
- [ ] Long-press options menu: new UI entirely. Three sections in order — (1) Units & Core Info: F/C, MPH/KPH, Feels Like on/off, UV on/off, Wind on/off, Visibility on/off, Cloud Cover on/off; (2) Safety: Severe Alerts on/off; (3) Visual Flourishes: Moon Phase on/off, Sunrise/Sunset Gradient on/off, Live Condition Skin on/off. All 11 toggle states persist per-device via `localStorage` across reloads.
- [ ] Temperature unit tap vs. long-press distinction: tapping °F/°C is currently the *only* toggle mechanism and it's persistent. Needs to become two separate things — long-press menu F/C selection = persistent default (survives reload); tapping the unit symbol directly = temporary "peek" only, reverts to the long-press default on next load, does not touch the stored setting.
