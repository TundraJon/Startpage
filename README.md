...
- [ ] **Remaining open sub-question, not resolved by this message — flagging rather than assuming:** when `sunGradient` is off but `liveSkin` is on, should the "basic" day/night background be a single flat color per state (e.g. `FULL_DAY_SKY`/`DEEP_NIGHT_SKY`'s own colors, no twilight blending), or should it still transition smoothly through the existing twilight window — just with simpler, flatter colors than the full multi-stop keyframe treatment the gradient toggle adds?
