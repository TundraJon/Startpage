...
- [ ] **Feasibility note on the fallback plan (font-size reduction):** the user's proposed 3-step technique (shrink the font by 1px, tighten line spacing by 1px, and shrink *just the second line's* font by another 1px, for ~3px saved) can't be done literally in CSS — there's no way to target only the second line of auto-wrapped text without JS splitting the name into separate elements to detect the wrap point, a much bigger change than a style tweak. **Pending user decision:** either (a) reduce the font-size uniformly by ~2px total (simple CSS change, same ballpark of space saved, applies evenly instead of only to line 2), or (b) build the more involved JS-based per-line approach. Not decided yet — do not build either until specified.
...
- [ ] **Requested approach, verified precisely with real `getBBox()` measurements across all 12 condition icons used in the graph:** reduce the label offset by exactly 5px (`p.y + 25.5` → `p.y + 20.5`, per request) **and** shrink `ICON_BOTTOM` from `53` to `44` (curve/icons sit higher, `ICON_TOP` and the SVG's own 70px height both stay untouched, per request — "reduce the graph area, leave the space alone"). This combination gives **3.5px of clean clearance** below the lowest label in the worst case, verified against every icon glyph in the set (not just one), with a very minor (~0.5px, likely imperceptible) reduction in the gap between icon and label — a natural side effect of pulling them closer together as requested.
