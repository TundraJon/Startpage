...
- [ ] **Depends on the tile usage statistics entry in the Build Queue** (`createdAt`/`lastUsedAt`/`useCount` per tile) — these three are why that data is being collected, per the user. None of this is buildable before that lands; this entry defines what each consumer needs once it has.
...
- [x] **2. Long-press multi-select, extended to categories, with range-select, select-all/clear, and scale/lift selection feedback — built (Build Log 50).** Also folded in along the way: the tile-menu popup is retired (long-press goes straight into select mode and its bottom bar now), and Move Entry's trigger changed to a continuous long-press-then-drag gesture (its underlying mechanics are unchanged).
- [x] **3. Create UI — built (Build Log 48).** Home's header is now sticky with a "+ Tile"/"+ Category" entry point. Category creation currently sets `stripeColor: null` — item 4 (color picker) below is the next natural piece to build on top of this.
- [ ] **4. Color picker for primary (top-level) categories.** `--stripe-color` exists today only as a hand-typed inline style per category in index.html — no picker UI anywhere. Needs: a preset palette + custom color UI, storing the chosen color in the new category data model (item 1), and applying it the same way `--stripe-color` already works. Scoped to primary/top-level categories per the doc, picker-wise. **Subcategory color — decided:** subcategories inherit the top-level category's chosen color, lightened. Exact lightening amount/method (e.g. a fixed `color-mix()`/HSL-lightness step, deeper subcategories lighter still vs. one flat step for all depths) not yet worked out — to be settled when this is promoted and actually built.
- [x] **5. Cut + Paste (move only, not Copy — see precedence note above) for both tiles and whole category subtrees — built (Build Log 50).** `moveCategorySubtree` reparents a whole category subtree in one operation via the same select → navigate → Paste pattern tiles already used, with a cycle guard against moving a category into itself or one of its own descendants.
- [ ] **6. Drag-and-drop reordering of tiles within the same category — already built.** This is exactly Move Entry's same-grid reorder path (`reflowWithinCurrentGrid`, nearest-tile-center detection, live visual feedback via `move-dragging`/`move-grabbed` classes) — no new work, just confirming it already satisfies this requirement as written.
- [ ] **7. Search across all tiles, with auto-navigate + scroll-into-view + glowing highlight.** Nothing like this exists today — the only search bar in the app is the external web-search form (`#search-form`, submits to Google/Bing). This needs: an index over every category's stored tiles (likely built the same way the "Reports" idea above would enumerate them — DOM-driven or storage-key-driven), a results UI showing title + category path, and on selecting a result: computing that tile's category chain and calling `openCategoryPath` on it (reusing the existing navigation), then `scrollIntoView` plus a new CSS pulse-glow animation (light blue border, ~1.5–2s fade) — no existing animation to reuse here, would be new.
- [ ] **8. Rename — single exists for both tiles and categories now (Build Log 50), batch is still new.** Single tile rename (via the bottom bar's ✏️, `openTileRenameFor`) and single category rename (via 🔧 Edit, `openEditCategory`) both exist. Batch rename with pattern support (prefix/suffix/counter/find-replace) across multiple selected tiles or categories is still entirely new — no pattern-rename logic anywhere in the codebase today.
- [ ] **9. Delete — single and batch both exist now (Build Log 50, tiles and categories alike, easter-egg double-confirm intact); soft-delete/trash still doesn't.** The bottom bar's 🗑️ (`deleteSelected`) handles any current selection, one tile or many, or a category subtree. Soft-delete/trash ("if feasible" per the doc) is still a bigger, separate design — nothing about it decided.
- [ ] **11. Export/Import JSON backup.** Doesn't exist. Once the category data model (item 1) exists, this is: serialize the full tree + all `category-tiles-*` storage into one JSON blob for export, and a matching import/restore path with validation. Needs a decision on UI (Settings menu is the natural home, already has other data-adjacent controls like the WeatherAPI key field).
- [ ] **12. Undo for the last major action (move/delete/reorder).** Doesn't exist anywhere today — every mutation (`saveCategoryTiles`, tile/category delete, move) writes straight to localStorage with no history. Needs a small undo-stack design (what's captured, how far back, cleared when) before it's buildable — not decided yet.
- [ ] **13. UX polish items already substantially covered:** large touch targets, dark mode (already built), smooth animations (Move Entry, category expand/collapse already animate) — these aren't new asks, just confirming the existing app already leans this direction. Haptic feedback (`navigator.vibrate`, where supported) is the one genuinely new, small addition in this group.
- [ ] **Open, not yet decided:** exact shape of "current location" for create (item 3) when nothing is open; the exact lightening step for inherited subcategory color (item 4); the undo stack's design (item 12); soft-delete/trash's design (item 9). None of these block writing the category data model (item 1) first, which is the one everything else is downstream of.
