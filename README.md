...
- [ ] Per the user: on the "Add" panel (`#create-menu-overlay`'s `.tile-menu-actions`, opened via Home's global "+"), each of the three buttons (Tile, Category, Grouping) currently renders as 2 lines — `.tile-menu-icon` (🔗/📁/➕) on its own line, then a combined `.tile-menu-action-label` reading "+ Tile" / "+ Category" / "+ Grouping" as the second line (index.html). The user wants 3 separate lines instead: the icon, then a standalone "+" on its own line, then just the name ("Tile" / "Category" / "Grouping") — splitting the label's baked-in "+ " prefix out into its own line rather than sharing a line with the name. Applies to all three buttons.

### Bug: Category Color's "None" doesn't clear the saved color

- [ ] Per the user: on a top-level category (confirmed with Sample Category A / `test-a`), selecting None for the Category Color and pressing Save doesn't remove the color — it keeps whatever was already saved. Confirmed the hex text field itself does correctly clear (shows the `#RRGGBB` placeholder, meaning it's genuinely empty), so the disconnect is between that and the value actually written on Save.
- [ ] **Tried to reproduce directly and couldn't** — a scripted "click None → verify field empty → Save → check storage" sequence worked correctly (stored `stripeColor` became `null`, the stripe visually cleared). So this isn't simply "the code never writes null," at least not in that exact path.
- [ ] **Most likely root cause, found via code review — not confirmed, best available theory:** the None handler (`createColorPicker`, script.js) clears the hex text field and calls `onChange(null)`, but never touches the native `<input type="color">` swatch sitting next to it (`#category-color-native`) — that element keeps whatever color it last displayed. If anything afterward causes that native color input to fire its own `input` event with its stale retained value (a real risk specifically on touch devices when a native color-picker widget loses/regains focus — not something a scripted click can reproduce, which is why direct testing didn't catch it), its handler would silently overwrite `pendingStripeColor` back to the old color immediately before Save, undoing the None. This fits the reported symptom exactly: the text field visibly clears, but the save doesn't stick.
- [ ] **Not yet fixed** — the fix, if this theory holds, is to also reset the native color input (to some neutral value) inside the None handler so it has nothing stale left to re-fire, and/or guard against a color-input `input` event overriding an explicit None until the user picks something new again. Needs to actually be tried on a touch device to confirm before considering it closed.
...
### Groupings become fully centralized in the Edit Category dialog — supersedes Build 60's grid-based grouping editing entirely
...
- [x] **Groupings list + one shared icon row, per the user (corrects an earlier wrong reading of mine — per-row icons):** the Groupings section shows the list of current groupings (e.g. Free/Paid/Attribution) same as today, with **one shared row** of 🔺 🔻 ➕ ✏️ 🗑️ underneath the whole list — not one icon set per row.
- [x] **Selection model, per the user, "similar to how the categories work":** tap a grouping in the list to select it (single-select, highlighted). The shared icon row's buttons enable only once something's selected, mirroring the Reorg Tree Tool's own toolbar exactly. 🔺 disabled when the selected grouping is already first in the list; 🔻 disabled when it's already last — same as the Reorg Tool's Category Up/Down disabling at either end of its sibling list.
- [x] **Move semantics — decided by the user: whole-block move.** Up/Down moves the grouping's label *and every tile under it* together, as one unit, past the adjacent grouping's block — the same way a top-level category moves with its entire subtree in the Reorg Tool. Not a single-slot swap.
- [x] **Long-press a divider in the grid — decided by the user, this is the big one:** no longer enters the grid's own grouping-select-mode/bottom-bar at all. It now **opens the Edit Category dialog directly** (for that divider's category), with that specific grouping already selected/highlighted in the Groupings list — exactly mirroring how long-pressing a category header now opens the Reorg Tree Tool pre-selected (Build 60). The user's own words: "remove the grid's own [grouping] editing functions in favor of this new centralized procedure."
- [x] **This fully supersedes/removes, not just adds alongside:** Build 60's `'grouping'` `selectMode` kind, the select-action-bar's ✏️/🗑️/➕ behavior for it, `enterGroupingSelectMode`, `groupingDividerEl`, the in-grid `startGroupingInlineRename`/`.tile-divider-input`/`.tile-divider-selected` mechanism, and the previously-logged (never-built) grid-based Grouping Up/Down entry that used to sit here — all of that grid-side machinery goes away in favor of this dialog being the one and only place groupings get created, renamed, reordered, and deleted. Tile drag-to-reorder itself, and dragging a tile across a divider to change which group it's in, are untouched — this is only about the grouping's *own* long-press/select/edit path, not tile placement.
- [x] **Long-press on empty grid space — decided by the user: removed outright.** "Should not be a thing" — this is not rerouted anywhere, the gesture itself goes away. Creating a grouping only ever happens through the Edit Category dialog now (its own ➕, or via Home's "+ Grouping" below).
- [x] **Home's global "+" menu's "+ Grouping" — decided by the user:** opens the Edit Category dialog (for `currentLocationId()`, same targeting the other two "+" menu options already use) with the dialog's own ➕ already actioned — i.e. it lands the user straight into the same "new grouping, ready to inline-rename" state the dialog's ➕ itself produces, not just the dialog sitting there waiting for another tap.
- [x] **✏️ in the dialog's shared icon row — confirmed: inline editing, "for now."** The user's own qualifier — inline rename is the intended interim behavior, not necessarily the final word on it forever, but it's what to build.
- [x] **Resolves an ambiguity — decided by the user:** "After this next update, there should only be one place to edit a category, and edit a grouping, and edit a tile." Category and Tile already satisfy this today (`openEditCategory` and `openTileRenameFor` each have exactly one call site already). Grouping does not yet, even under this spec as originally written — ✏️'s "inline editing" was ambiguous between editing the name right there in the dialog's own list row, vs. reusing Build 60's `focusGroupingInLiveGrid`/`revealLiveGridForGrouping` trick to reveal the live grid underneath and edit the divider's label there instead. The one-place rule settles it: ✏️ must rename the grouping directly in the dialog's list row, self-contained — no revealing or scrolling to the live grid for editing purposes at all. `focusGroupingInLiveGrid` and `revealLiveGridForGrouping` are therefore also fully removed, not reused, alongside everything already listed above as superseded.
