...
### Reported: clouds all start at the left edge on refresh instead of pre-scattered (intermittent — parked)

- [ ] **User report:** on refresh, all clouds appear to start together at the left edge and only spread apart over time as their differing speeds separate them, instead of appearing already scattered across the sky. Device: Pixel 7 Pro, Chrome (not Safari/WebKit — rules out an earlier hypothesis).
- [ ] **Investigated, not reproduced on demand; confirmed intermittent by the user.** Seen 3 times total; ~12 fresh refreshes in a row afterward did not reproduce it. Two mechanisms were tested and ruled out via headless Chromium: (1) initial-spawn stagger — sampled cloud positions frame-by-frame from the very first paint across no-key/testing-panel, fresh-fetch, and fresh-cache-reload paths (up to 9 clouds) — always already scattered in the first frame; (2) respawn clustering — tracked every cloud (re)spawn over a 60-second window (10 clouds) — respawns land one at a time, spread out over the whole window, never clustered.
- [x] **Likely trigger identified by the user:** the reset was observed specifically after switching away from the browser tab (to talk to Claude) and back — clouds were stacked at the left and moving right when the tab regained focus, mid-session, with no page reload involved. This fits known mobile-Chrome behavior of throttling or resetting animation/compositor state for backgrounded tabs to save battery. Confirmed via code search that nothing in script.js listens for `visibilitychange`/`focus`/`blur` at all — if the reset is happening, it's the browser's own animation engine doing it while backgrounded, not anything in this app's JS, which also explains the intermittency (only backgrounding the tab long enough to trigger the browser's own throttling would do it, not a plain refresh).
- [ ] **This changes the fix direction.** The previously-proposed fix (stagger the respawn/rebuild path, aimed at the live-data double-render theory) doesn't address a browser-level animation reset on tab-visibility-regain. The more likely fix: listen for the Page Visibility API (`document.addEventListener('visibilitychange', ...)`) and, when the page becomes visible again, re-run the same randomized-negative-delay logic already used on initial spawn against the *existing* clouds (or replace them the same safe way `spawnCloud(oldCloud)` already does) — re-asserting scattered positions rather than trusting the browser to have preserved them through the background period.
- [ ] **Parked at the user's request** — not worth chasing further right now given how rarely it reproduces. If it recurs, worth confirming it's specifically tied to backgrounding the tab (vs. some other trigger) before building the visibilitychange-based fix above.
- [ ] **Candidate fix, ready if/when revisited:** give the respawn/rebuild path the same small randomized stagger the initial batch already gets, instead of a hard zero-delay reset on every full clear-and-rebuild — cheap, addresses the double-render theory and the user's own suggested fallback (stagger creation) without reintroducing the original jitter bug (still a freshly created element per cloud, just a nonzero starting offset).
