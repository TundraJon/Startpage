...
- [ ] **Still undesigned:** what the dropdown rows show (title + category path, presumably); and what selecting a match does — needs at minimum computing that tile's category chain and calling `openCategoryPath` on it, then `scrollIntoView`, and whether a glow/highlight effect on landing (mentioned in earlier discussion) is still wanted or was specific to the old full-screen-results framing.
