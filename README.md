...
- [ ] **No more `bounceHeightMult` — 25% of stones just fall twice as fast instead.** Replace the height-multiplier field entirely. At creation, ~25% of hailstones get `speed: (7 + Math.random() * 2) * 2` (double fall speed) instead of the normal `7 + Math.random() * 2`. That per-stone `speed` value becomes the stone's own bounce "energy" — no separate multiplier anywhere; a stone that fell faster naturally bounces harder because energy in equals energy out.
- [ ] **Vertical/horizontal split via true trigonometric decomposition, not a linear split.** At the peak of a bounce, a stone's energy (its own fall `speed`) splits into `vertical = energy * cos(angle)` and `horizontal = energy * sin(angle)` (angle measured from vertical, matching the existing `bounceAngle` convention). Verified against the user's own worked examples: at 0° a normal stone (energy 8) gets 8 vertical / 0 horizontal; at 45° it splits to ~5.66 / ~5.66 (not the linear 4/4 the user initially proposed — confirmed with them that the trig split, where the two components satisfy `vertical² + horizontal² = energy²` rather than summing arithmetically to the total, is what they actually want). A boosted stone (energy 16) at the system's actual max angle of 60° gets `16×cos60° = 8` vertical / `16×sin60° ≈ 13.86` horizontal — still less vertical than a normal stone bouncing straight up (8 at 0°), so an extreme angle and an extreme height still can't both max out on the same stone, without needing any manual patch to prevent it — the energy split does that automatically.
- [ ] **Implementation shape:** replace the current `hop = Math.sin(min(bounceT/10,1)*PI) * 8 * heightMult` / `x = bounceFromX + hop * Math.tan(angle)` pair with something like: `energy = p.speed` (captured once at the start of the bounce, alongside the angle roll), `vertical = energy * Math.cos(p.bounceAngle)`, `horizontal = energy * Math.sin(p.bounceAngle)`, then apply the existing `Math.sin(min(bounceT/10,1)*PI)` time-based easing curve to each of `vertical`/`horizontal` independently (so the whole bounce still arcs up-and-down smoothly over its ~14-frame duration, just with two components instead of one derived from the other).
...
- [ ] **Flash interval — flat 5–10s gap, dropping the double-flash roll entirely.** There's no intensity concept at all — just "thunderstorm." Replace the current 8–20s gap + ~32% chance of a quick second flash with a single flat 5–10s interval between flashes (the range originally proposed as "medium," now just the one and only behavior). No separate "double-flash" scheduling needed — the new flash-duration flicker (next point) organically produces a similar "multiple pulses" look within one flash.
- [ ] **Flash duration — variable with per-frame flicker.** Replace the fixed 100–150ms single static opacity with a 100–1000ms duration whose alpha is re-rolled every frame for the whole lifetime of the flash (`flashAlpha * (0.4 + Math.random()*0.6)`, peak `flashAlpha` itself randomized 0.85–1.0), so it visibly flickers throughout rather than holding one steady brightness.
- [ ] **Rendering mechanism — keep the DOM div for the ambient wash, not a canvas fillRect.** Decided this is actually better than the reference's canvas-fillRect approach, not just easier: the existing `.weather-skin-flash` div sits as the topmost layer (above the floating cloud sprites), so the whole widget including clouds washes with light; moving the wash onto `precipCanvas` would put it *underneath* the cloud sprites (wrong — they wouldn't react to the flash), and would also entangle its draw-order with rain/snow/stars already drawn that frame. Keep `flashDiv`, just update its opacity every frame instead of once to get the flicker. The bolt itself still has to be canvas-drawn either way (no way to render a jagged procedural shape via CSS) — draw it on `precipCanvas` as today.
- [ ] **Bolt chance — 25% per flash**, up from the current 10%.
- [ ] **Bolt targeting — merged from both versions.** Endpoint (strike point) biased to 75–100% down the canvas (`h*0.75 + Math.random()*h*0.25` — the reference's range, replacing the current 40–90%). Start point stays independently random along the top edge (the current approach, `w*(0.15 + Math.random()*0.7)`), *not* the reference's "offset from the endpoint" start logic.
- [ ] **Bolt shape — unchanged, do not adopt the reference's version.** Keep the current 5–8 segments with a 40% chance of a small fork/branch. The reference's 7–9 segments with no branching was considered and explicitly rejected.
- [ ] **Bolt rendering — unchanged, single stroke pass.** Keep the current one-pass stroke (single color, single glow). The reference's two-pass glow+bright-white-core rendering was considered and explicitly rejected.
- [ ] **No intensity concept at all — simplified after discussion.** Light/medium/heavy tiers were considered (WeatherAPI's thunder-related codes only describe rain intensity, not actual lightning/storm intensity, so deriving light/heavy from them was never a genuine signal anyway) and explicitly dropped: there's just "thunderstorm," using the flat 5–10s interval above. No `thunderIntensity` field, no Testing Panel intensity controls, no per-tier branching anywhere — `scheduleLightning()` needs no lookup table, just the one fixed range.

### Cloud overlay can never occlude stars (or anything else drawn above it), and is too subtle to see at night

- [ ] **Root cause #1, confirmed via DOM inspection — wrong stacking order.** `renderWeatherSkin()` appends `.weather-skin-overlay` (the cloud tint) to `weather-skin` *before* `precipCanvas` (the layer stars/rain/snow/hail/fog/lightning are drawn on), and neither has a `z-index`, so plain DOM order applies: the overlay paints first (bottom), the precip canvas paints after (on top). This means the cloud tint can never visually darken or hide anything on the precip canvas, no matter how opaque it gets. Verified directly: at 100% cloud cover, nighttime, the overlay's own opacity measured exactly `1` (fully opaque, matching the expected 1:2 ratio — night cloud opacity is `cloudPct/100`) — yet the star canvas still measured stars at 247/255 (~97%) brightness, completely unaffected.
- [ ] **Root cause #2, confirmed via computed style — too subtle at night regardless.** Separately, even where the overlay *is* the topmost visible thing (over the plain background), its actual rendered color at night measured `rgb(3, 8, 32)` against a background of `rgb(2, 6, 23)` — a few-unit difference that's nearly imperceptible. The night formula (`multiplier = 1 + nightBasePct/100`, brightening clouds relative to the sky) is mathematically doing what it's supposed to, but a 40% *relative* change on colors this dark barely registers in absolute terms — the same category of issue as the fog fix in Build Log 16.
- [ ] **Not the same as the floating cloud emoji sprites** (`.wx-skin-cloud`), which already render and animate correctly (confirmed: 5 appeared at 50% cloud override) — this is specifically about the gray/tinted overlay meant to darken the whole sky proportional to cloud%.
- [ ] **Fix direction, per the user — give stars their own layer, correctly positioned behind the clouds (better than an opacity-multiplier workaround).** Stars belong on "the sky layer" — behind the cloud tint — while rain/snow/hail/fog correctly stay in front of it (precipitation falls toward the viewer, in front of the cloud backdrop) and don't need to move. Concretely: add a new dedicated canvas for stars only, inserted between `.weather-skin`'s own background (the sky gradient) and `.weather-skin-overlay`, so the stacking becomes sky background → new star canvas → cloud overlay → existing `precipCanvas` (rain/snow/hail/fog/bolt, unchanged) → floating cloud sprites → flash. Occlusion then happens for free through ordinary alpha compositing at any cloud opacity, rather than needing a separate formula kept in sync with the overlay's math. Star creation/drawing (currently interleaved with everything else in `rebuildConditionParticles`/`stepConditionSkin`) needs to move onto this new canvas — same per-frame clear/resize/DPR pattern as `precipCanvas`, just a second instance sized and positioned earlier in the stack. The overlay's own too-subtle-at-night color (root cause #2) still needs a separate tuning pass, likely following the same "raise it until it's actually visible against the real widget" approach used for fog.
