...
### Weather widget: larger drifting clouds pop in instead of entering fully offscreen

- [ ] Reported: bigger clouds are sometimes visibly "popping" onto the left edge instead of sliding in from offscreen. Measured and confirmed: `.wx-skin-cloud`'s offscreen buffer is a flat `left: -15%` of the widget's width (~247px wide in testing → ~37px buffer), but cloud width is a fixed rem value from the Build Log 9 height-correlated sizing (1x–2.5x, i.e. 1rem–2.5rem) that doesn't scale with the widget — and emoji glyphs render noticeably wider than their font-size (a measured 2.39rem cloud was ~48px wide, about 1.25× its nominal size). At `left: -15%`, that cloud's right edge lands at roughly `-37px + 48px ≈ +11px` — already inside the widget when the animation starts. Smaller clouds (~20px wide) stay fully hidden at the same offset; only the larger end of the 1x–2.5x range is affected, matching what was reported.
- [ ] A bigger flat percentage (e.g. `-25%` instead of `-15%`) would mask the symptom at the current widget width but isn't a real fix: cloud width is fixed in rem/px while the buffer is a percentage of container width, so the two don't scale together — the same bug would resurface on a narrower widget or if the largest-cloud size ever changes. The correct fix computes each cloud's actual offscreen offset from its own rendered width (e.g. `left: calc(-1 * var(--cloud-w))` / `calc(100% + var(--cloud-w))` in the keyframe, with `--cloud-w` set per-cloud as a CSS custom property from its measured width, or the animation driven by JS instead of a single shared `@keyframes` used by every cloud regardless of size) — guaranteeing full concealment at both ends of the drift regardless of cloud size or widget width.

### Weather widget: cloud lifecycle overhaul (re-randomize height per lap, no mid-flight despawn)

- [ ] Currently each cloud's height (`top`), size, and speed are set once at creation and never change — the same element just loops the same CSS keyframe (`left: -15%` → `115%`, `animation-iteration-count: infinite`) forever, confirmed by code read-through. New requirement: every time a cloud completes a lap and wraps back around to enter from the left, it should get a freshly randomized height for that new lap (and, per the existing height-correlated formula from Build Log 9, a size/speed recalculated from that new height — `f = topPct / 50`, `size = 2.5 - 1.5*f`, `duration = 17 + 43*f`), not the same fixed lane forever.
- [ ] **Real bug this also fixes:** currently `renderWeatherSkin()` (script.js:634) blanket-removes *every* `.wx-skin-cloud`/`.weather-skin-overlay` element (line 647) and rebuilds the whole cloud set from scratch on every call — and it's called from 8 different places (every settings toggle, the testing panel's time/cloud/opacity controls, etc.). That means any of those changes currently makes all visible clouds vanish and instantly respawn elsewhere, mid-flight, with no transition. The new requirement explicitly rules this out: clouds must never disappear mid-flight. When cloud % drops and fewer clouds are needed, the excess should finish their current lap and exit normally off the right edge, then simply not respawn — removed at the same moment they'd otherwise wrap and re-enter, not before. When cloud % rises and more are needed, new ones are created via the normal creation path so they naturally start their life at the left/offscreen edge like any freshly created cloud already does — no special-casing needed there beyond just adding to what's active.
- [ ] Implementation shape: needs per-cloud persistent state across `renderWeatherSkin()` calls instead of the current full teardown-and-rebuild — e.g. an array of live cloud elements (with an "expiring" flag) held outside the function. On each settings change, the function should diff the target count against the current live count: create `(target - current)` new clouds if short, or flag `(current - target)` existing ones as expiring if there's a surplus (removing only the surplus's worth, not all of them, and not immediately).
- [ ] Detecting "a cloud just completed a lap" needs a real hook, since a single shared `@keyframes` on `animation-iteration-count: infinite` gives no JS callback by default. The `animationiteration` DOM event is the right tool — it fires on a looping CSS animation each time one iteration ends and the next begins, which is exactly the "wraps back around to the left" moment described. Each cloud would get one `animationiteration` listener attached at creation: if the cloud is still needed, re-randomize its `top`/size/duration inline styles for the next lap; if it's flagged expiring, remove it from the DOM right there instead.
- [ ] Worth verifying once built (not assumed to just work): whether updating a CSS animation's inline `animation-duration` from inside its own `animationiteration` handler causes any visible restart/flicker versus a clean continuation into the next lap — browsers can behave subtly differently here, so this needs an actual on-device/visual check, not just a computed-style check.
- [ ] Interacts with the offscreen-entry-width item directly above this one: a respawned cloud re-enters the same way a freshly created one does, so whatever fix that item lands on (per-cloud pixel-width-based offscreen offset) needs to apply equally to respawns, not just initial creation.

### Weather widget: dynamic (background-aware) text color

- [ ] This is the concrete answer to the night-readability problem noted in the very first queue item above (condition-specific effects / text-stroke idea): instead of a fixed text color, compute it live from the current background so it stays readable through the sunrise/sunset transition, not just at the two extremes. User's rule: light/near-white background → black text; dark/near-black background → medium gray text.
- [ ] The relevant color source already exists: `computeSkyColors(now)` (script.js:519) returns `{ top, bottom }` hex colors every render via the sunrise/sunset keyframe interpolation, and `renderWeatherSkin()` applies it as `weatherSkin.style.background` at a given opacity over the widget's own (white, by default) background. The luminance check must be based on the actual *composited* color the text sits against, which is **three** layers, not two: white base → sky gradient (at its opacity) → cloud overlay tint (`.weather-skin-overlay`, flat `#4b5563`, opacity `cloudPct / 2 / 100`, covers the full widget same as the gradient does). The cloud layer pulls the composite darker on its own — matters even in full daylight at high cloud %, not just at night — so it can't be left out of the calc or the color swap will fire at the wrong point (or miss cases entirely) whenever cloud cover is heavy. Using only the raw uncomposited `sky.top`/`sky.bottom` values (ignoring white base and cloud tint) would be wrong for the same reason.
- [ ] **Open dependency:** whether the white base layer stays in the design at all is still undecided (see "corrected layer stack" above — currently `weatherTestState.whiteBgOn`/`gradientOpacityPct`, testing-panel-only) — tabled by the user for now, to be decided after spending more time looking at it live. This item is blocked on that decision, since it changes what "composited color" even means (white+gradient+clouds vs. just gradient+clouds at full strength).
- [ ] Real-world confirmation: user shared a screenshot (post Build Log 9, temp text now `var(--fg-muted)` matching the unit color) where the blue→tan sunset-ish gradient made the "88°" temp and "91°/72°" hi/lo nearly unreadable — medium gray on a mid-tone background. Confirmed this is exactly the case the two-state design (black on light backgrounds, medium gray on dark) already handles, not a reason to change the design: that gradient reads as light/warm overall, so the rule as planned would already resolve to black there. The gradient strength in that screenshot was confirmed to just be wherever the testing panel happened to be set, not a statement about the intended production look — doesn't resolve the white-base dependency above. Also re-confirmed: medium gray (not white) stays the answer for dark/near-black backgrounds, no change from the original spec.
- [ ] Needs a luminance function (standard relative luminance from the composited RGB, or a simple weighted average) and a threshold to pick between the two text colors. **Decided:** hard cutoff, not a crossfade — the color snaps from black to medium gray (or back) the instant the composited background crosses the threshold, no transition animation on the text color itself. Exact threshold value still to be picked/tuned live via the testing panel.
- [ ] **Decided:** one widget-wide color, not per-row — a single luminance value computed once per render (matching how the gradient itself is already applied as one top-to-bottom sweep, not evaluated separately per text row) drives all the widget's text at once, as it currently stands.
