...
### Live Condition Skin: cloud overlay derived from sky brightness (exact formula)

- [ ] **Supersedes the earlier "hardcoded darker hex per condition" version of this item** — user gave a precise formula instead, which also resolves the previously-flagged open question (derive from the actual sky color, not a separately-tuned flat dark gray) and the cross-cutting `WX_CLOUD_TINT_RGB` staleness issue (there's no longer a separate hardcoded tint constant to fall out of sync — the composite calc should just reuse the same derived value the overlay itself uses).
- [ ] **Replaces the fixed `#4b5563` gray entirely.** The cloud overlay's color is no longer independent of the sky — it's the current sky color's own brightness, scaled by a multiplier. "Current sky color" = the same single representative value already used elsewhere for this purpose (`lerpColor(sky.top, sky.bottom, 0.5)`, i.e. the midpoint average already established for the text-color composite calc) — reusing that convention rather than introducing a second one.
- [ ] **Day/night split, exact boundary:** daytime = start of the sunrise transition through end of the sunset transition (i.e. everywhere `computeSkyColors()` does *not* return the flat `DEEP_NIGHT_SKY` value — matches the existing `SUNRISE_SEC ± TWILIGHT_HALF` / `SUNSET_SEC ± TWILIGHT_HALF` boundaries already in the code, no new threshold needed). Nighttime = only the flat deep-night period.
- [ ] **Brightness multiplier** (applied to the sky color's own brightness to get the cloud tint's brightness):
  - Daytime base: sky brightness × 60% (40% darker).
  - Nighttime base: sky brightness × 140% (40% brighter) — the mirror image of the daytime base.
  - Per-condition further shift (subtracted from whichever base is in effect, same signed amount both day and night): Snow 0%, Hail 0%, Light Rain −10%, Heavy Rain −20%, Thunderstorm −40%. (Partly Cloudy/Overcast/Fog/Clear Day/Clear Night: 0%, no condition-specific effect — they just get the plain day/night base.)
  - Worked examples from the user: daytime Heavy Rain at any cloud % → sky brightness × (60% − 20%) = **40%**. Nighttime Heavy Rain → sky brightness × (140% − 20%) = **120%**, i.e. "20% brighter than the sky color" — matches the user's own stated example exactly.
  - Formula: `multiplier = 1 + (daytime ? -0.40 : +0.40) + (conditionShiftPct / 100)`, where `conditionShiftPct` is the negative per-condition value above (0 for snow/hail/none). Multiply each RGB channel of the sky-average color by this multiplier (clamp 0–255).
- [ ] **Opacity — day/night now branches, wasn't gated before:** daytime stays exactly as it is today, `cloudPct / 2 / 100` (50% of cloud cover). Nighttime becomes `cloudPct / 100` directly — full 1:1 with cloud cover, double the daytime rate. Worked example: night Heavy Rain at 100% cloud cover → 100% opacity, matching the user's own example. Opacity is **not** condition-dependent — only day/night + cloud % drive it; the brightness multiplier above is where conditions matter.
- [ ] Implementation: `.weather-skin-overlay`'s `background` (script.js:866, currently a fixed string) needs to become a computed color from this formula each render, and its `opacity` (script.js:867, currently the unconditional `cloudPct / 2 / 100`) needs the day/night branch. `WX_CLOUD_TINT_RGB` (script.js:655) can likely be deleted outright — the text-color composite calc (script.js:898) should just use whatever RGB value this same formula produces for the current moment, computed once and shared between the overlay's actual paint and the composite math, rather than a separate constant.
- [ ] User's own closing caveat — "or something like that" — signals these exact percentages (60/140% base, 10/20/40% per-condition shifts) are a strong starting proposal, not necessarily final.
- [ ] **Testing panel controls, requested explicitly — both sliders to change the numbers and displays to see the result live:**
  - Sliders: "Day cloud base darkness" (0–100%, default 40), "Night cloud base brightness" (0–100%, default 40), "Light Rain shift" (0–100%, default 10), "Heavy Rain shift" (0–100%, default 20), "Thunderstorm shift" (0–100%, default 40). Snow/Hail stay fixed at 0 per spec (no slider), unless it turns out those should be adjustable too once seen live.
  - Displays, updating live as sliders move or the condition/time/cloud% changes: the resulting brightness multiplier as a percent (e.g. "Cloud brightness: 40% of sky"), the resulting opacity as a percent, and a small color swatch showing the actual computed cloud tint color (plus its hex value as text) — so the exact color is visible, not just the abstract percentages. Also worth showing which branch (Day/Night) is currently active, since that's derived from the time override and not otherwise obvious while tuning.
  - These join the existing "Preview Condition Skins" section of the Testing Panel rather than a new section, since they only matter once a condition (or the plain day/night base) is being previewed.
...
- [ ] **Actual fix, verified in an isolated repro:** on `animationiteration`, don't mutate the existing cloud element — remove it and insert a brand-new `<span class="wx-skin-cloud">` in its place (`weatherSkin.replaceChild` or equivalent) with fresh randomized `top`/`fontSize`/`animationDuration`/`--cloud-w`, no `animation-delay` set at all (a freshly-created element just starts a normal, uninterrupted animation from its own `from` keyframe — nothing to reinterpret, since there's no prior running timeline to disrupt). Repro comparison: mutating an existing element's duration on each iteration produced 19 rogue iterations in 4 seconds with positions scattered from -39px to 233px on a 300px box; the same test using element-replacement instead produced exactly 3 clean iterations in 4 seconds (matching the ~1s average duration), landing at precisely `-40px` (fully offscreen, matching `--cloud-w`) every single time.
- [ ] Implementation note: the new element needs its own `animationiteration` listener attached (pointing at itself for its own future respawn), same pattern as today, just via element replacement instead of `randomizeCloud(cloud)` mutating in place. `randomizeCloud()` itself can likely stay close to as-is (it already returns/sets values on whatever element it's given) — the change is at the call site: create-and-replace instead of mutate-in-place.
