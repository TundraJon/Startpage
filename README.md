...
- [x] **Batch category Cut+Paste — decided by the user: acceptable to lose.** Category select mode (and with it, multi-select range-select for categories) goes away entirely along with this change; single-category moves via the Reorg Tool's own drag are enough.
...
- [x] **Nesting depth — decided by the user:** the color picker only ever applies to a top-level ("primary") category. Subcategories are never individually colored — they automatically get progressively lighter shades of their primary ancestor's color, the deeper they nest. No picker of their own; purely computed. Needs a real design pass on the actual lightening algorithm (e.g. step the color's HSL lightness up per depth level, clamped) — not specified beyond "progressively lighter."
- [x] **Verified via code review, relevant to the above:** a regular category header today only ever shows its `stripeColor` as a thin 3px top-border accent (`.category-header`, styles.css) — the header background stays the app's neutral surface color and the name text stays the standard foreground color always. So there's no text-contrast risk for regular categories today, and none introduced by this feature either, as long as that accent-border treatment (not a full background fill) is what stays — this entry doesn't ask for that to change.
- [x] **Home — decided by the user: yes, gets the same picker**, but applied differently — to the *entire* header background, not a thin accent border, since Home's header already fully fills with color today (fixed, not user-set: `--home-header-bg`/`--home-header-fg`). Lives in Home Settings' own dialog, presumably as a third section mirroring Edit Category's own Sort Tiles → Groupings → Color layout.
- [x] **Text/background contrast — real problem, correctly flagged by the user, and decided:** since the header background can go dark or light, white text won't always work. **Text color and background color get picked independently** (two pickers, not one, with auto-contrast left out of scope) — the user's own proposed fix, adopted as the design.
- [x] **Technical note found via code review, not a question — just something to get right at build time:** `--home-header-bg`/`--home-header-fg` are currently *global* CSS custom properties, also reused as the general accent color all over the rest of the app (button/input borders, checkmarks, etc. — many unrelated spots in styles.css). Home's header can't just repoint those two shared variables when its color changes, or every one of those unrelated UI elements would recolor along with it. Needs its own dedicated variable pair (or direct inline styling) scoped specifically to Home's own header, independent of the accent variables everything else still relies on.

### Grouping editing redesign: long-press + inline rename + bottom icon bar, replacing the popups

- [ ] Per the user: editing a grouping (Build Log 58) is clunky — today it's either long-pressing the exact divider bar, or Edit Category → Groupings list → ✏️, and either way opens a whole separate popup dialog just to rename or delete one line of text. **Confirmed direction:** long-pressing a divider selects it and shows the same bottom icon action bar Organize Mode already uses for tiles/categories, instead of opening a dialog. Natural fit as a third `selectMode` kind (`'grouping'`), reusing the existing bar/timeout/Clear-button infrastructure rather than inventing a new mechanism.
- [ ] **Icons on that bar, per the user:** ✏️ — turns the divider's own label into an inline-editable text field right there in the grid (no popup); 🗑️ — delete, keeping today's existing named-item confirmation dialog (that one stays modal on purpose, it's destructive); **➕ (new, per this message)** — adds another new divider, without leaving this same bar.
- [x] **What ➕ does — decided by the user:** inserts a new divider right after the selected one and drops straight into that same inline-rename state with an empty label ready to type. Fully popup-free.
- [x] **The two existing Build 58 entry points — decided by the user: replaced entirely, not kept as secondary paths.** Long-pressing empty grid space and the Groupings list's ✏️ inside Edit Category both go away as popup-opening triggers. Long-press-empty-space still creates a grouping, but now the same way ➕ does — inserted and dropped straight into inline-rename, no dialog.
- [ ] **Not actually decided, flagging so it isn't mistaken as such:** what happens to the Groupings *list* inside Edit Category once its ✏️ no longer opens a popup. A grouping only reachable by scrolling to it in the grid and long-pressing would be a real regression for a long category, so the list likely still needs to exist as a way to *find* one — my own proposed default (not confirmed by the user) is that ✏️ there scrolls to and selects that divider, entering the same select+bar+inline-rename state, rather than the list disappearing outright.
- [x] **Home's global "+" button also needs a third option, per the user (new, this message):** the create-menu (`#create-menu-overlay`, today just "+ Tile" / "+ Category") gains "+ Grouping" — adds a grouping to whatever category is currently open (`currentLocationId()`, same targeting the other two already use), appended at the end and dropped into inline-rename the same way ➕ and empty-space-long-press do.
