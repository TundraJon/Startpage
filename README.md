...
## Build Log 76 (completed)

### Help Overlay content: all 7 accuracy fixes from the three-pass audit applied

- [x] **Section 3** — no longer claims long-pressing the sun/moon button does anything; now points to Settings → Theme for Auto mode.
- [x] **Section 4** — the "+" bullet now describes it as opening a menu (not a single direct action); the Home Settings bullet now covers all three of its real sections (Sort, Groupings, Home's own colors), not just sorting.
- [x] **Section 6** — no longer describes a per-category "+" (removed in Build 49); now correctly describes Home's single "+" → "+ Tile", acting on whichever category is currently open.
- [x] **Section 7** — now mentions both real entry points for adding a grouping (Home's "+ Grouping" menu option, and the category's own edit screen), plus the long-press-a-divider-in-the-grid shortcut straight to editing it.
- [x] **Section 9** — the toolbar bullet no longer implies ➕ Category can add "alongside" a selected row; now correctly distinguishes ➕ Category (always top-level) from ➕ Subcategory (nests under the selected row).
- [x] **Section 10** — the search-result example now uses the real separators (`>` between categories, `_` before a grouping) instead of arrows throughout.
...
### First-run choice: Full vs. Lite starter content — same app, different seed data

Per the user: once this link goes out for real, everyone who opens it for the first time gets the full seed set (categories/tiles) written to their own `localStorage`, same as today, and can then customize freely from there ("control their own destiny on their own device"). Wants a first-run option for a lighter starting point instead — not a separate version or build, purely which seed data gets written on that very first load. Design discussion resolved with the user:

- **Mechanism:** a one-time modal, shown before any seeding happens (ahead of `loadCategoryTree()`/`loadCategoryTiles()`'s existing migration-flag logic, `script.js`), offering **Full** (today's seed set) or **Lite** (a small new starter set — roughly 6 tiles plus one example category with one example subcategory, real-sounding names, not literally "dummy"). Whichever is picked drives which seed data those existing migration functions write on that first run; every migration flag, storage key, and all app functionality afterward is completely unaffected — this is a one-time fork in what gets written, nothing else.
- **Blocking issue found while scoping this, not a new ask — resolved:** `CATEGORY_SEED_DATA`/`TILE_SEED_DATA` (`script.js`) still contain the original dev/test placeholder categories (`test-a` through `test-e`, "Sample Category A" etc.), self-flagged in the code as disposable. Per the user: they'll supply the real full starter category/tile set directly (upload incoming) — this build waits on that file rather than guessing at replacement content. **Not yet built — still waiting on the upload as of this log entry.**
- **Resolved: the first-run choice does not get remembered anywhere.** Per the user: "we're just going to go back to the beginning and ask them to make a choice" — no stored flag for which one was picked. This simplifies Reset-to-Factory-Defaults below: it doesn't need to know or restore "whichever was originally chosen," it just re-triggers the same first-run picker.
- Fireworks/confetti on the choice — discussed and set aside per the user, not part of this item.

### Reset to Factory Defaults, with a RESET-typed confirmation + a backup nudge

Per the user, to give Lite users (or anyone) an escape hatch back to a clean starting point, since there's no such thing today.

- **Entry point — not yet decided.** Natural candidate: a new destructive action in Settings, likely at the bottom near Backup, matching the app's existing pattern of grouping related actions together.
- **Resolved: single confirmation tier, not multiple.** Per the user, explicitly: "we're just going to go with the reset with the backup nudge for now" — the multi-round "are you REALLY sure this time" idea was floated as a joke, not something to build. **Confirmation reuses the existing shared confirm dialog (`openTileConfirm`, `script.js`)** — same one Remove Category and Backup Import already use — but needs one new behavior neither of them has: the existing typed check is case-*insensitive* (`.trim().toLowerCase() !== 'yes'`); per the user, Reset's typed word is **"RESET," case-sensitive, all uppercase required** — a deliberately stronger, distinct tier from the existing lowercase-friendly "yes" used elsewhere, specifically so routine-delete muscle memory can't trigger it. `openTileConfirm`'s `opts` will need a new flag for this (e.g. `requireTypedReset` alongside the existing `requireTypedYes`), not a reuse of the existing case-insensitive check.
- **Resolved: backup nudge shown unconditionally**, every time Reset is opened, right inside the same confirm dialog — reusing the exact same export path `#backup-export-btn` already uses (`script.js`), not new export logic and not new "has anything changed" detection.
- **What "factory defaults" restores to:** per the resolved item above, Reset doesn't restore a remembered choice — it wipes storage and re-shows the same first-run Full/Lite picker, same as a brand-new device.
- **Explicitly deferred, per the user's own "I don't know" — not part of this item:** multiple named/versioned backup slots. The existing single-file Export/Import already covers "keep more than one backup" today — a person can export repeatedly and name the files themselves on their own device, no app change needed for that. Revisit only if it turns out people actually want in-app version management later.
