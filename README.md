...
- [x] The "does changing `animation-duration` mid-loop cause a visible flicker" question flagged when this was queued is resolved structurally rather than needing a guess: the update happens exactly at the iteration boundary (inside the `animationiteration` handler itself, before the browser paints the new iteration), so the new duration/position only ever takes effect for laps that haven't started yet — not applicable mid-flight.
