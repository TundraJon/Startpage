...
- [ ] Remove the ~8px off-white gap between categories (currently `.category { margin-bottom: 10px }`, which shows the page's off-white background through the gap). Replace with a thin line/border between categories instead, like a table-cell border — no empty space, just a hairline separator. Goal is a noticeably more compact page overall.
