...
- [ ] **Fix direction:** rain/snow/hail/fog reasonably belong *in front of* the cloud tint (precipitation falls toward the viewer, in front of the cloud backdrop) — so a wholesale re-stacking of the overlay above the entire precip canvas isn't obviously right either. Stars are the one thing that should be genuinely *behind* clouds and occluded by them. Simplest targeted fix: multiply star opacity by an additional occlusion factor derived from the same `cloudOverlayOpacity()` value already computed for the tint (e.g. `starOpacity *= (1 - cloudOverlayOpacity)`), so at 100% night cloud (overlay opacity 1) stars go fully invisible, and at partial cloud they dim proportionally — matching the 1:2 ratio the user already has in mind, without restructuring the canvas/DOM stacking or affecting how rain/snow/hail/fog render. The overlay's own too-subtle-at-night color (root cause #2) would still need a separate tuning pass, likely following the same "raise it until it's actually visible against the real widget" approach used for fog.
