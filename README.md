...
### Reported: clouds all start at the left edge on refresh instead of pre-scattered

- [ ] **User report:** on refresh, all clouds appear to start together at the left edge and only spread apart over time as their differing speeds separate them, instead of appearing already scattered across the sky.
- [ ] **Investigated, not yet reproduced.** The initial-spawn stagger logic (`renderWeatherSkin()`, script.js, unchanged since Build Log 13) gives each newly spawned cloud a random *negative* `animation-delay` up to its own duration, so it should start already mid-journey rather than at 0s/left-edge. Tested via headless Chromium across three realistic paths — no API key (testing-panel-driven), a fresh live fetch, and a refresh hitting the 15-minute cache, up to 9 clouds — sampling cloud positions frame-by-frame from the very first paint each time. In every case clouds were already spread across the full width in the first captured frame; no lineup-then-spread behavior observed.
- [ ] **Leading hypothesis, unconfirmed:** Safari/WebKit has a known history of not retroactively repositioning a CSS animation when `animation-delay` is set via JS immediately after the element starts animating — it can just start the delay countdown from zero instead of snapping to the correct mid-cycle position, which would look exactly like the reported symptom. Chrome/Firefox handle this correctly (matches the negative test result above). Needs confirmation of which browser reproduces it before deciding the fix — likely a WebKit-safe variant of the same approach (e.g., creating each cloud already paused, forcing a reflow, setting its delay, then starting it) rather than switching to staggered creation, since creation-time staggering would reintroduce the "clouds pop in one at a time from the edge" look the negative-delay approach was built to avoid.
