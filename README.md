...
- [x] **8. Dropped — not wanted, per the user.** Batch rename with pattern support (prefix/suffix/counter/find-replace) across multiple selected items. Single rename (tile and category) stays exactly as built — only the batch/pattern part was ever open here, and it's off the table now.
- [x] **9. Dropped — not wanted, per the user.** Batch/multi-category delete and soft-delete/trash. Single delete (tile, single or batch, and category one-at-a-time via the Reorg Tree Tool) stays exactly as built — only these two undesigned extensions were ever open here.
- [ ] **11. Export/Import JSON backup — now tracked in the Build Queue, not here.** Logged there after reconciling against the Two-Instance Mechanism — Per-Device Storage spec; this line stays only as a cross-reference so this entry doesn't look like it still owns an open item.
- [x] **12. Dropped — not wanted, per the user.** Undo for the last major action (move/delete/reorder). No undo stack, no history capture — not being built.
- [x] **13. Dropped — not wanted, per the user.** Haptic feedback (`navigator.vibrate`) was the one open item in this group; the rest (touch targets, dark mode, animations) was already covered and stays as-is.
- [ ] **Open, not yet decided:** exact shape of "current location" for create (item 3) when nothing is open; whether category range-select/batch-select and batch category Cut+Paste/delete (all dropped in Build 60, per items 2/5 above) are wanted back in some form, now that the Reorg Tree Tool exists as their replacement — not raised again since Build 60, worth confirming is still the accepted tradeoff rather than assuming so indefinitely.
