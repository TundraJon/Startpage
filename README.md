...
### Hourly Forecast panel: redesigned layout — precip-alert icon, condition icon, temperature graph, precip %

- [ ] **Context — a design discussion following user feedback that the current precip graph (Build 28) is confusing:** the user didn't recognize what the blue line at the bottom represented, then referenced an AccuWeather-style hourly panel design and worked through several decisions with the assistant, including a correction to the ordering below, before landing on this spec. Not a resize/relabel — a real change in what data is shown, how, and in what order.
- [ ] **Final per-column vertical order, top to bottom (corrected from an earlier draft of this same spec):**
  1. **Precip-alert micro icon + colored dot** — the same condition emoji already used for that hour (`weatherIconForCode(hour.condition.code)`, explicitly "our same emoji," not a new icon set), small size, paired with a colored dot keyed to the hour's precip % (`hourlyPrecipPct(hour)`, already computed as `max(chance_of_rain, chance_of_snow)`): **≥80% red, 60-79% orange, 30-59% yellow, <30% nothing displayed** — but the slot's vertical space must always be reserved (fixed-height, present in every column regardless of content) so columns don't jump/misalign depending on which hours qualify. Thresholds chosen to track the National Weather Service's own probability-of-precipitation wording bands (confirmed with the user in discussion): NWS shifts language at 20% "Slight Chance," 30% "Chance" begins, 60% "Likely" begins, 80% stated as fact — the 30/60/80 cutoffs align with those real tiers, not an arbitrary pick.
  2. **Time label** (existing `.hourly-time`, "Now" for the current hour else a parsed 12-hour label) — unchanged, just now sitting below the new alert icon instead of at the very top.
  3. **Condition icon** (existing `.hourly-emoji`, `weatherIconForCode(hour.condition.code)`) — unchanged, position unchanged relative to time.
  4. **Temperature, shown via a shared line graph** (not a per-column number) spanning the full strip width beneath the row of condition icons — replaces the removed precip graph (see below). Y-axis scaled so the *highest* of the 12 fetched hourly temps sits at the top of the graph and the *lowest* sits at the bottom (dynamic range, not fixed) — same general SVG-polyline mechanism as the old precip graph, sourced from `hour.temp_f` through the existing unit-conversion path instead. Target height ~200px, called out by the user as a rough starting point to be tuned once seen live, not a firm spec. **Assumption to confirm once built:** numeric temperature labels shown at each point on the graph (matching the reference image), since the graph is now the *only* place temperature appears per hour — flag if a plain unlabeled line was actually wanted instead. No second "feels like" line — user agreed to skip that (adds a second curve + legend to a panel meant to stay compact), can revisit later.
  5. **Precip % text**, only when greater than 0% (existing behavior, unchanged) — now sits at the very bottom, under the graph, instead of directly under the condition icon as before.
- [ ] **Remove the old precipitation graph entirely:** delete `#hourly-precip-graph` (index.html), `renderHourlyPrecipGraph()` and its call site (script.js), and its CSS (`.hourly-precip-graph`, styles.css) — replaced by the temperature graph above. **User's original reasoning for removing it:** the old graph was a single overall shape spanning the whole panel width with no visible tie to the individual hour columns above it.
