...
- [ ] The relevant color source already exists: `computeSkyColors(now)` (script.js:519) returns `{ top, bottom }` hex colors every render via the sunrise/sunset keyframe interpolation, and `renderWeatherSkin()` applies it as `weatherSkin.style.background` at a given opacity over the widget's own (white, by default) background. The luminance check should be based on the actual *composited* color the text sits against — i.e. blending the sky color with white at whatever opacity is in effect (currently `weatherTestState.whiteBgOn`/`gradientOpacityPct`, still testing-panel-only, not a permanent setting) — not the raw uncomposited `sky.top`/`sky.bottom` values, or the color swap will fire later/earlier than the background actually looks light or dark.
- [ ] Needs a luminance function (standard relative luminance from the composited RGB, or a simple weighted average) and a threshold to pick between the two text colors; where exactly to draw that threshold, and whether it should be a hard cutoff or its own short crossfade (to avoid the text color itself "popping" instantly mid-transition) is an open implementation question worth checking live via the testing panel rather than guessing.
- [ ] Also open: since the gradient goes top-to-bottom and text rows sit at different vertical positions in the widget, a single widget-wide luminance value is an approximation (this matches what the user asked for — not per-row color) — worth confirming that's sufficient once seen live, rather than needing per-row luminance sampling.
