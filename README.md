...
### Star twinkle: too big/soft, and the whole animation model should change from a smooth pulse to real scintillation

- [ ] **Behavior model — replace the continuous sine pulse with an event-driven flicker, per user's description of real twinkling.** Right now every star continuously breathes in and out via `tw = 0.4 + 0.6 * (0.5 + 0.5*sin(phase))` — a smooth, always-moving pulse, which reads as "everything gently pulsing" rather than twinkling. Real scintillation is intermittent: a star sits steady at its own baseline brightness almost all the time, then occasionally (randomly, a few seconds apart, asynchronous per star) does one brief (under a second) dip or flare and returns to steady. Agreed approach: give each star (a) a **baseline opacity**, split into two rough populations — most stars usually-bright with occasional brief dims, a smaller set usually-dimmer with occasional brief brightens; (b) a randomized **next-flicker timestamp**; when reached, ease briefly away from baseline (dip for the usually-bright population, flare for the usually-dim one) over well under a second, then return to steady and roll a new random delay before the next flicker. This also resolves the earlier-diagnosed frame-rate-dependency issue (`s.phase` advancing per invocation rather than per elapsed time) for free, since flicker timing would be driven off real timestamps (`ts`, already passed into `stepConditionSkin`) rather than a per-frame phase counter.
- [ ] **Star size — smaller.** Current radius is `0.8 + Math.random()` (0.8–1.8px), close enough to snow's `1.5 + Math.random()*1.5` (1.5–3px) fill-circle rendering to read as the same shape at a glance. Fix: shrink meaningfully (e.g. ~0.4–1.0px) so stars read as fine points rather than snowflake-sized dots.
- [ ] **Star sharpness — root cause found, affects the whole precipitation canvas, not just stars.** `precipCanvas.width`/`.height` are set directly from `getBoundingClientRect()`'s CSS-pixel dimensions with no `devicePixelRatio` scaling anywhere in the file (confirmed via search — zero references to `devicePixelRatio`). On a high-density phone screen (the user's Pixel 7 Pro likely runs a DPR of 2.5–3+), the canvas backing store is rendered at a much lower resolution than the physical screen and then stretched to fit, which softens/blurs everything drawn on it — rain, snow, hail, fog, the flash, and stars alike. Fix: size the canvas backing store to `w * devicePixelRatio` × `h * devicePixelRatio`, keep its CSS size at `w`×`h`, and scale the drawing context by `devicePixelRatio` once after resize — standard canvas HiDPI pattern. This will sharpen the whole condition-skin canvas, which is a reasonable side effect given the user's asking for stars specifically to look "sharper."
- [ ] **Star color — currently all pure white, should have some variety.** `precipCtx.fillStyle` is hardcoded to `rgba(255,255,255,${tw})` for every star. Real night skies show mostly white/blue-white stars with an occasional visibly warmer (yellow/orange) or cooler (blue) one. Fix: assign each star a color at creation time from a small weighted palette — heavily biased toward white/near-white, with a minority tinted blue-white, pale yellow, or pale orange — and use that star's own color (still modulated by its twinkle brightness) instead of the fixed white fill.

### Remove the Clear Day sun rays

- [ ] **User request:** the rotating sun-ray flourish drawn for the Clear Day condition skin (`stepConditionSkin`, the `c.has('clearDay')` block — 5 rotating `rgba(255,230,150,0.35)` lines radiating from the top-right corner) looks weird and should be removed entirely. Clear Day would then have no canvas flourish of its own, relying on the sky gradient/background brightness alone to read as clear — matching how Clear Night would still keep its stars.
...
- [ ] **Fix:** add a `document.addEventListener('visibilitychange', ...)` handler that, when the page becomes visible again, re-applies the same randomized-negative-delay stagger the initial batch already gets to the existing clouds (reusing the same safe `spawnCloud(oldCloud)` element-replacement path already used for respawns, not a mutate-in-place, to avoid reintroducing the original jitter bug).
- [ ] **Scope, confirmed with the user:** purely additive — a new listener, touching only `.wx-skin-cloud` elements. Doesn't fire on a normal load/refresh (only on an actual hide→show transition), doesn't touch weather data fetching, the 15-minute cache, or any other widget. Since clouds/weather only ever update on refresh anyway, this listener firing is not expected to be visibly noticeable in normal use — it's a safety net for the specific backgrounded-tab case only.

### Testing Panel should only cover the bottom half of the screen

- [ ] **User request:** the Testing Panel overlay currently covers the full screen (it shares the generic `.help-overlay` class — `position: fixed; inset: 0`, centered — with the Settings and Help overlays, `styles.css:608`), so picking a testing option hides the weather widget itself. To see the effect of a selection, the user currently has to scroll up and tap close each time. Wants the Testing Panel specifically to only occupy the bottom 50% of the screen, leaving the widget visible above it while adjusting sliders/checkboxes.
- [ ] **Implementation note:** needs a Testing-Panel-specific override rather than a change to `.help-overlay` itself, since Settings/Help should stay as full-screen centered modals — only `#testing-panel-overlay` (and its `.help-panel` child) should reposition to dock against the bottom half of the viewport (e.g. `top: 50%; height: 50%;` instead of `inset: 0`, panel filling that strip instead of a small centered card).
...
- [ ] **Consequence for Live Condition Skin's background:** currently `.weather-skin`'s actual background (`weatherSkin.style.background`, script.js:924-931) is only ever set `if (weatherSettings.sunGradient)` — with that toggle off, there's no background at all regardless of `liveSkin`, which is what produced the "white/transparent instead of dark at night" behavior from the earlier discussion. Needs a basic day/night-derived background (dark at night, lighter/blue by day) that renders whenever `liveSkin` is on, independent of whether `sunGradient` is *also* on — the fancy multi-stop transition coloring stays specific to the gradient toggle, but a plain day-or-night background should no longer require it.
- [ ] **Consequence for Clear Day/Clear Night flourishes — resolved:** "Clear Day"/"Clear Night" are not a manual selection at all — they're **inferred automatically whenever no other condition is selected/active**, using the real day/night determination (`isDaytime()`, already used by the Build Log 13 cloud-brightness formula). So the sun-rays/twinkle-stars flourish (whichever matches the current real day/night state) becomes the *default* appearance of Live Condition Skin, not something a person has to opt into — it's what you get when `liveSkin` is on and nothing else (rain/snow/hail/thunderstorm/fog) is checked. This also answers what "clear" means relative to the other conditions: it's the absence of any of them, not its own explicit signal.
- [ ] The two "Clear Day"/"Clear Night" checkboxes in the Testing Panel's "Preview Condition Skins" picker are therefore redundant and should be removed — unchecking everything already produces the clear-sky appearance automatically; there's nothing left for those two entries to do.
- [ ] **Already consistent, no change needed:** the Build Log 13 cloud-tint brightness formula (`computeCloudTint()`) already calls `computeSkyColors(now)` directly and derives its day/night branch from `isDaytime()`, independent of `weatherSettings.sunGradient` — this part already matches the clarified model, confirmed by re-reading the code just now.
- [ ] **Resolved:** when `sunGradient` is off but `liveSkin` is on, the basic background is a flat color per state (day / night), no twilight blending — it changes instantly at the day/night boundary rather than transitioning. The smooth multi-stop transition stays exclusive to the gradient toggle being on.
- [ ] **Star fade rule, given by the user:** twinkling stars should only render during actual night — and should fade in/out through the sunrise/sunset transition, but *only when the Sunrise/Sunset Gradient toggle is active*. With the toggle off, stars should just hard-cut on/off at the night/day boundary instead (no transition to fade along with).
  - **Convenient detail: the exact value needed already exists.** `computeSkyColors()`'s internal `xRatio` (script.js:615/619) is 0 at the night end of each transition window and 1 at the day end, consistently for *both* the sunrise and sunset windows (confirmed by reading both branches — sunrise's `xRatio` counts up from 0→1 as SUNRISE_KEYFRAMES goes night-color→day-color; sunset's is computed as `1 - progress`, landing on the same 0=night/1=day convention against SUNSET_KEYFRAMES). So the star fade multiplier is just `1 - xRatio` — no separate calculation needed, same value already driving the color blend.
  - **Implementation note:** `xRatio` is currently local to `computeSkyColors()`, not exposed. Needs factoring out into a small shared helper (e.g. `computeNightFactor(now)` returning that same 0–1 value continuously across sunrise/day/sunset/night, not just during the two transition windows) so both the color interpolation and the star-fade multiplier read from one place instead of duplicating the boundary math.
  - Each star's existing individual twinkle (`conditionStars`, script.js:750/789, the per-star randomized-phase opacity pulse) should multiply by this fade factor rather than being replaced by it — stars keep twinkling individually, the whole group's ceiling brightness rises/falls with the transition.
