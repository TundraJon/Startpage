...
### Star twinkle: too slow, too big/soft, and should dip darker at the low end
...
- [ ] **Star darkness — refined spec.** Not a shift of the whole range — the *peak* twinkle brightness (current ceiling of `1.0`, full white) is fine as-is. Only the *dim* end, as each star twinkles down, should go several shades darker than the current floor of `0.4`. Fix: keep the ceiling at `1.0` but drop the floor substantially (e.g. down toward `0.05`–`0.1`) in the `tw = floor + (1 - floor) * (0.5 + 0.5*sin(phase))` formula, so the low points of the twinkle read as much dimmer while the bright peaks stay unchanged.
