...
- [ ] **User report:** on refresh, all clouds appear to start together at the left edge and only spread apart over time as their differing speeds separate them, instead of appearing already scattered across the sky. Device: Pixel 7 Pro, Chrome (not Safari/WebKit — rules out the earlier hypothesis below).
- [ ] **Investigated, not reproduced; confirmed intermittent by the user.** Seen 3 times total; ~12 fresh refreshes in a row afterward did not reproduce it. Two mechanisms were tested and ruled out via headless Chromium: (1) initial-spawn stagger — sampled cloud positions frame-by-frame from the very first paint across no-key/testing-panel, fresh-fetch, and fresh-cache-reload paths (up to 9 clouds) — always already scattered in the first frame; (2) respawn clustering — tracked every cloud (re)spawn over a 60-second window (10 clouds) — respawns land one at a time, spread out over the whole window, never clustered.
- [ ] **Remaining untested variable:** the double-render on real page load — an immediate placeholder-data render followed by a second full clear-and-rebuild once live geolocation + the WeatherAPI fetch actually resolve. That second pass's timing depends on real GPS lock and network latency, which is exactly what couldn't be simulated here (testing used instant mocked responses). This is the leading remaining suspect for why it'd show up rarely rather than every time.
- [ ] **Parked at the user's request** — not worth chasing further right now given how rarely it reproduces. If it recurs, worth noting exact timing (instant on refresh vs. a reset a few seconds in) to confirm/rule out the double-render theory above.
