...
Kept deliberately separate from the existing Google/Bing web-search bar (`#search-form`), per the user — a shared input risked accidentally triggering a tile jump instead of a web search, or vice versa.

- [ ] **Entry point — decided by the user:** a 🔎 icon button in `.home-header-actions`, to the left of `#create-btn` ("+"), so the Home header row reads 🔎 + ▲. A new, separate button, not a repurposing of any existing one.
- [ ] **UI — decided by the user: a popup, not a full-screen view** (unlike Settings/Reorg Tool) — the natural fit is this codebase's existing `.help-overlay` pattern already used for Add Tile/Edit Tile/tile blurb/etc., rather than the `#settings-view`/`#reorg-view` full-screen pattern. Contains a text input with a live autocomplete dropdown of matching tiles appearing as the user types (not a separate "submit and see results" step).
- [x] **Index source — decided: DOM-driven, not storage-driven.** `document.querySelectorAll('.category[data-category-id]')` + the tiles inside each. Reasoning: `wireTileGrids` renders every category's tiles into the DOM eagerly at load — collapsed categories are just `[hidden]`, their tiles are still there — and every mutation (add/rename/delete/badge/reorder/grouping) already patches the live DOM element alongside its storage write, so the DOM is never stale relative to storage by the time a search would run. Storage-driven (scanning `category-tiles-*` keys directly) was rejected: it would also surface orphaned tiles left behind under a since-deleted category, which search should never offer since selecting one would navigate nowhere.
- [x] **Dropdown row content — decided by the user:** full category chain, then the tile name at the end — e.g. `Games > Computer > Role Play / Free > Wolfenstein`. Note this example includes a *grouping* name ("Free," a Visual Grouping Header divider inside the "Role Play" category), not just category nesting — worth a small formatting decision at build time on how to visually distinguish a grouping segment from a real category-chain segment (the user's own example used `/` for the grouping vs. `>` for categories), but the content itself — chain, optional grouping, tile name — is settled.
- [x] **Selection behavior — confirmed: category-chain nav + scrollIntoView is the baseline.** Compute the tile's category chain, call `openCategoryPath` on it, then `scrollIntoView`.
- [x] **Glow/highlight-on-landing effect — confirmed still wanted, per the user.** A new CSS pulse-glow animation on the landed-on tile (light blue border, ~1.5–2s fade, per the original description) — no existing animation in the codebase to reuse, this is new.
