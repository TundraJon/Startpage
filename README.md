...
- [ ] **Consequence for Clear Day/Clear Night flourishes:** sun rays and twinkling stars currently only appear when manually checked in the Testing Panel's picker (`weatherTestState.conditionSkins`, a Build Log 12 simplification, documented at the time as a known gap). Per this clarification they should instead auto-trigger from the same real day/night determination (the existing `isDaytime()` used by the Build Log 13 cloud-brightness formula) rather than requiring manual selection — being at night should be sufficient on its own, without a person having to also tick "Clear Night."
...
- [ ] **Open sub-questions, not resolved by this message — flagging rather than assuming:**
  - When `sunGradient` is off but `liveSkin` is on, should the "basic" day/night background be a single flat color per state (e.g. `FULL_DAY_SKY`/`DEEP_NIGHT_SKY`'s own colors, no twilight blending), or should it still transition smoothly through the existing twilight window — just with simpler, flatter colors than the full multi-stop keyframe treatment the gradient toggle adds?
  - Do "Clear Day"/"Clear Night" stay as manual entries in the Testing Panel's picker at all once they're automatic in real use (useful for override/preview regardless of actual time), or does auto-triggering make those two specific checkboxes redundant?
  - How does "auto-triggering Clear Day/Night" interact with cloud % and the other precip conditions once real weather data exists — is "clear" simply "no other condition active," or does it need its own explicit signal from the API's condition code?
