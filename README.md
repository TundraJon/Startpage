...
- [ ] **Still undesigned:** the index this reads from (DOM-driven `document.querySelectorAll('.category[data-category-id]')` vs. storage-key-driven scan of `category-tiles-*` — same open question as Planner 2's "Reports" idea); what the dropdown rows show (title + category path, presumably); and what selecting a match does — needs at minimum computing that tile's category chain and calling `openCategoryPath` on it, then `scrollIntoView`, and whether a glow/highlight effect on landing (mentioned in earlier discussion) is still wanted or was specific to the old full-screen-results framing.
