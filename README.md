...
- [ ] **Reproduced and root-caused directly** (not guessed) — confirmed via a live reproduction that the tile's `getBoundingClientRect()` goes to `{w:0, h:0}` and `offsetParent` becomes `null` (both are the standard signs of "inside a `display:none` ancestor") the moment a different, unrelated header gets hovered:
  1. The dragged tile is only ever visually offset via a CSS `transform` (`followPointer`, script.js) — it's never actually removed from wherever it currently lives in the category tree (`dragInfo.currentGrid`, initially the source grid). It only gets reparented into a different grid when the pointer moves directly over a real, visible `.tile-grid` area (`enterGrid`, called from `processDragPosition`'s non-header branch) — hovering a *header* never touches `currentGrid` at all.
  2. If the whole drag happens while the pointer stays over headers (opening subcategories along the way, exactly as reported) rather than dropping into a grid's tile area, the tile silently stays parented in its **original** source grid the entire time.
  3. Build 42's global single-open-path navigation collapses whatever else was open whenever a *different* header is hovered — including, with no special case, the category the tile is currently (invisibly) still sitting inside. Collapsing sets `hidden` on that category's content, which cascades `display: none` down to everything inside it — the dragged tile included.
  4. A `transform` does nothing to protect an element from a `display: none` ancestor — the tile is fully invisible the instant this happens, even though the drag itself doesn't break: `dragInfo` keeps updating normally, pointer events keep firing, and moving back down over any real visible grid triggers `enterGrid`, reparenting the tile into it and making it reappear — matching "until I moved it back downward again" exactly.
- [ ] **This directly contradicts an explicit assumption in Build 42's own planning notes** — "the dragged tile itself is already detached from normal layout... so its origin category collapsing around it doesn't disrupt the drag." That's true for the *drag mechanics* (dragInfo, pointer tracking, drop logic all keep working correctly), but false for *visibility*: being offset by a transform is not the same as being detached from the render tree, and nothing currently stops an ordinary hidden ancestor from swallowing the tile whole.
- [ ] **Decided: never let a drag's own current location collapse — a deliberate, standing exception to the single-open rule while a drag is active.** The user chose this over the alternatives (a top-level "ghost" drag layer; proactively relocating the tile before its grid collapses), explicitly accepting the tradeoff — two things can be visibly open at once during a drag (wherever the tile currently sits, plus wherever hover navigated elsewhere), on purpose, so the tile's origin stays visible for reference even while exploring other categories.
  - **Generalized from "keep the original open" to "keep wherever the tile currently is open," since those aren't always the same category:** if the drag does cross into a different grid mid-drag (`enterGrid` fires when hovering directly over a visible grid, updating `dragInfo.currentGrid`), the tile is no longer in its original spot — pinning only the *original* source wouldn't protect it there. Pinning `dragInfo.currentGrid`'s owning category (whatever that currently is) covers the original-spot case too, since before any reparenting happens, current and original are the same category.
  - **Technical approach for the build:** `renderOpenPath` (script.js) needs to treat a second, separate chain as "on path" alongside the real `openPath` array whenever a drag is active — the ancestor chain of whichever category currently contains `dragInfo.currentGrid` (via `categoryAncestorChain`, already exists). That pinned chain's categories get `contentEl.hidden = false` the same as normal on-path ones. One extra wrinkle: if the tile is sitting directly in a with-subcategories category's own grid (not a subcategory's), that category's `ownGridEl.hidden` needs to be forced visible too even when it isn't the real path's leaf — otherwise the own-grid itself (a separate hidden-gated element one level below `contentEl`) would still swallow the tile. The pinned chain does *not* need its own collapse button or leaf semantics — it's a visibility-only exception, not a real navigable position.
  - **Also needs:** `renderOpenPath` is called from several places already (`openCategoryPath`, `collapseLeafCategory`, `collapseAllCategories`, and now also needs a call whenever `dragInfo.currentGrid` changes via `enterGrid`, so the pinned chain updates immediately when the tile crosses into a new grid) — and the drag ending (`finishTileDrag`/`cancelTileDrag`) needs one final `renderOpenPath()` call to drop the pinned exception once the drag is over, falling back to whatever the real `openPath` alone shows.
