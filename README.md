...
- [ ] User's own closing caveat — "or something like that" — signals these exact percentages (60/140% base, 10/20/40% per-condition shifts) are a strong starting proposal, not necessarily final. Worth keeping some way to preview/tune them live (per the slider request from the previous message) rather than treating the numbers as permanently locked in once built.
