...
### Phase 2 Part 4 spec: Whole-Structure Reorg Tree Tool — the two items above fold into this same round when it's built

Per the user: fold the two small items above into this next round rather than building them standalone. This is a genuinely new, substantial tool — nothing like it exists yet.

- [ ] **What it is:** a separate, full-screen view (not a modal overlay like every other dialog in the app today) showing the entire category/subcategory hierarchy site-wide as a flat, indented, draggable list — tiles never shown, only structure. Drag reorders siblings; dropping onto/beneath a different line re-nests (moving that category's own subtree along with it); a category can't be dropped under itself or its own descendant (snap back, no broken state); no artificial max-depth limit. Includes a "+ New Category" entry, immediately draggable within the same session. **Nothing touches the live site until Save** — all dragging/re-nesting/creation happens in a working copy; Cancel discards it entirely, Save commits it. Accessible from Settings ("Reorganize Categories" or similar).
- [ ] **Real conflict, already decided — flagging per the doc's own precedence rule:** Section 4 says "+ New Category" should live in this tool ("satisfies the Part 3, Section 9 reference"). But Build 54 already declined exactly this — "Create New Category placement (floating '+' in the Home header) — declined. Current behavior is exactly what's wanted." Two ways to reconcile, not decided: (a) the reorg tool still gets its own "+ New Category" entry as ONE MORE way to create a category, alongside the existing global "+" button (both stay); or (b) the reorg tool has no creation entry at all, deferring entirely to the existing "+" button, and this part of the spec is simply declined the same way the Part 3 mention was.
- [ ] **Home's position — the spec flags this itself as ambiguous, worth confirming rather than guessing:** Home should appear in the tree (it's a real top-level entry) but be locked at the top, non-draggable. Today Home isn't part of `categoryTree` at all — it's hardcoded as the first section in index.html and never participates in any reorder/drag mechanics — so showing it in this tree means special-casing a synthetic, always-first, non-draggable row. Open question: exactly what "locked" looks like in the UI (e.g. no drag handle / grayed out / a lock icon) isn't specified and wasn't decided elsewhere.
- [ ] **Not shown in the tree:** individual tiles, and the cosmetic Free/Paid-style grouping dividers within a category's own tile list — the spec says those have their own separate, simpler management (Insert/Rename/Delete Grouping) not part of this tool. Confirmed: that grouping-divider feature doesn't exist anywhere in the codebase yet (no matches for it in script.js/index.html) — it isn't built, so there's nothing for this tool to exclude in practice yet, just worth keeping in mind if it's ever added later.
- [ ] **Substantial reuse available** — the category data layer this tool needs already exists and is generic: `categoryChildren`, `categorySubtreeIds`, `categoryAncestorChain`, `moveCategorySubtree`, `newCategoryId`, `saveCategoryTree` (script.js) all already do exactly what a flat indented tree with drag-reparenting needs. The genuinely new work is: a full-screen (non-modal) view/markup pattern that doesn't exist anywhere else in the app yet, flattening `categoryTree` into an ordered, depth-indented list for rendering, a new drag-and-drop interaction for reordering/re-nesting that flat list (structurally different from both existing drag mechanics — tile Move Entry's same-category-only reorder, and category Cut+Paste's tap-to-navigate destination picking), and a working-copy/Save/Cancel state model so nothing touches real storage until Save.
