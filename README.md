...
### Placeholder test content: nested categories/subcategories (build BEFORE Move Entry below, per user's "do this first")

- [ ] **Spec supplied:** disposable test data (5 sample categories, A-E) explicitly meant to give the site real nested categories/subcategories to build and test the Tile Grid System, Category Headers & Accordion, and Phase 2 Move Entry against — since none of that nesting exists yet. Marked in the doc itself as temporary/removable, to be swapped for real content later.
- [ ] **Major scope note — this isn't just data, it requires new structural infrastructure:** subcategories don't exist anywhere in the current code — confirmed only 4 flat `.category` sections exist (Home/News/Shopping/Entertainment), no nesting concept in the DOM or the tile data model at all. The Category Headers & Accordion doc described subcategory *behavior* in the abstract (same header style, same accordion rules, only reachable once the parent is expanded) but never specified the actual data/DOM structure — that has to be designed now, for the first time, as part of this build.
- [ ] **Scope resolved with user:** the 5 test categories (A-E) render *alongside* the existing real categories (Home/News/Shopping/Entertainment) — nothing about those changes. Category C's "whole-structure reorg tree" reference is a forward reference to a document that doesn't exist yet — ignored for this build; everything else in the doc gets built.
- [ ] **Proposed data model (for review, not yet built):** a recursive tree instead of the current flat per-category list — each node is `{ id, name, tiles: [...], subcategories: [...] }`, so a subcategory is structurally identical to a top-level category, just nested one level deeper. Test tree, matching the doc exactly:
  - `Sample Category A` (flat, 3 tiles: Google, Wikipedia, GitHub)
  - `Sample Category B` (no direct tiles, 2 subcategories: `Sample Sub 1` [YouTube, Reddit, Amazon, Netflix], `Sample Sub 2` [Spotify, Apple])
  - `Sample Category C` (no direct tiles, 1 subcategory `Sample Sub A` [Microsoft, Yahoo] which itself has 1 sub-subcategory `Sample Sub A-1` [Twitch, Discord, Steam, LinkedIn, Pinterest, eBay — 6 tiles, tests multi-row wrap at a nested level])
  - `Sample Category D` (flat, 12 tiles spanning 3 rows — top-level multi-row wrap test)
  - `Sample Category E` (flat, 1 tile pointing at a nonexistent domain, to test the no-favicon fallback)
- [ ] **Rendering approach (for review):** recursive render function builds each node as a `.category`/`.subcategory` header+content block (same accordion mechanics at every depth, collapsed by default per the Category Headers spec); a node's own subcategory blocks render before its own `.tile-grid` (its direct tiles + "+" button) — this ordering is a judgment call, not something any spec pinned down, worth confirming before building.
- [ ] **Persistence:** extends the existing `category-tiles-<id>` localStorage scheme to path-based ids for nested nodes (e.g. `category-tiles-test-c-suba-1`), reusing the same migration/dedupe logic already built for the flat categories — no changes needed there, just more distinct category ids feeding the same functions.
- [ ] **Known limitation to flag on Category E:** the no-favicon fallback relies on the tile's `<img>` actually erroring out. Google's favicon proxy (used for all tiles, per the earlier resolved decision) almost always returns *some* image even for nonexistent domains, rather than failing outright — so this specific test tile may not reliably trigger the fallback path as intended. Will verify directly once built rather than assume either way.
- [ ] **Consequence for the already-queued Move Entry item below:** once this exists, Move Entry's cross-category cascading hover-to-expand (previously deferred to a future "flat version only" build, since nothing to cascade into existed) becomes fully buildable against Category C's real 2-level nesting. That full cascading behavior should be folded into the Move Entry build once this test content lands, rather than building Move Entry's flat-only fallback first and then redoing it.

### Phase 2 Editing System, Part 2: Move Entry (drag-and-drop) — per supplied spec doc

- [ ] **Build order note:** per the user, the placeholder test content above should land first — it turns Move Entry's cross-category hover-to-expand from "nothing to cascade into" into a fully testable feature against real nested categories, so the cascading behavior (spec Section 3) should be built for real here rather than deferred to the flat-only fallback originally planned.
- [ ] **Spec supplied:** full drag-and-drop system for repositioning tiles, triggered from the tile long-press menu's "Move Entry" item (currently present but disabled — script.js's tile menu, index.html:494 — since this doc hadn't shipped yet).
- [ ] **Conflict/gap identified, originally resolved as a flat-only build — now superseded:** the spec's cross-category move (Section 3) is built around drilling through nested subcategories ("Games → RPG → Battle Maps") — at the time this was first logged, the site had no subcategory concept at all. The placeholder test content item above resolves that gap directly, so build the *real* cascading hover-to-expand (Section 3 in full) against Category C's 2-level nesting, not the flat-only fallback (highlight-and-append-to-end with no cascading) originally planned as a stopgap.
- [ ] **Entry point:** re-enable the currently-disabled `#tile-menu-move` button; selecting it closes the tile menu and activates move mode for the grabbed tile's category.
- [ ] **Visual states, per spec Section 1:** all tiles in the active category get a subtle drop-shadow (new CSS, e.g. `.tile-grid.move-mode .tile`). The grabbed tile additionally gets a slight rotation + glow/highlight (e.g. `.tile.move-grabbed`), visually distinct from its shadowed-but-resting neighbors.
- [ ] **Auto-timeout, per spec Section 1:** move mode exits automatically after ~5s of no drag activity; the timer resets on any drag movement, not just on entry. Needs a `setTimeout`/`clearTimeout` pair reset on pointer-move events while move mode is active.
- [ ] **Same-category reorder, per spec Section 2:** dragging within the current category live-reflows the other tiles to make room; dropping finalizes the new order and persists it immediately to that category's `category-tiles-<id>` array (reordering the array, not touching ids/names/urls). Move mode stays active after a drop, allowing repeated repositioning until timeout or manual exit — explicitly not a single-drag-then-exit flow.
- [ ] **Cross-category move, per spec Section 3 (flat version, per the resolved gap above):** dragging onto a different category's header highlights it as a drop target; dropping moves the tile's data entry from the source category's array to the end of the destination category's array (removing it from source, appending to destination), then re-renders both grids. Per the spec's own explicit design, this does *not* attempt exact-position placement in the destination — a follow-up Move Entry session in the destination category handles fine-tuning, which "same-category reorder" above already covers once the tile lives there.
- [ ] **Exit conditions, per spec Section 4:** auto-timeout (above), or a tap anywhere outside any tile (including a category header tap, which should also exit move mode rather than fight with it — tapping a header to expand/collapse while in move mode should probably do both: exit move mode AND toggle the header, matching the spec's framing that this "should probably" happen, treated as the intended default). All move-mode visual indicators clear on exit.
- [ ] **Edge cases, per spec Section 5:** a drag that leaves the viewport entirely cancels gracefully — tile snaps back to its original position/order, no partial state. Move Entry works identically for favicon and fallback-text tiles (nothing here is icon-dependent). Hovering a flat category (no subcategories, which per the resolved gap is *every* category right now) during drag has no expand action to perform — dropping directly on its header still works.
- [ ] **Technical note:** `.tile` currently has `touch-action: manipulation` (styles.css) to keep tap/long-press gestures clean — this will likely need to switch to `touch-action: none` (or similar) specifically while move mode is active on that tile, so the browser doesn't fight custom drag tracking with its own scroll/pan handling. To be confirmed/tuned during the build, not now.
- [ ] **Verification checklist to run once built:** matches spec Section 6 exactly — move mode visual states enter/exit correctly, auto-timeout fires and resets correctly during active dragging, same-category drag reflows live and persists, move mode survives multiple repositions in one session, cross-category drop appends to destination end and removes from source, manual tap-away exit works (including via a category-header tap), off-viewport drag cancels cleanly with no partial state, works on both favicon and fallback-text tiles.
