...
- [ ] User's ask is comprehensive, not just the clock: "make sure everything on the screen obeys the dark/light toggle instantly." Audited every `getComputedStyle` call in script.js (the only mechanism by which a color could get baked in and miss a live update) — there are exactly two, both genuine instances of the same bug class, everything else is plain CSS `var(--fg)`/`var(--bg)`/etc. directly in the stylesheet, which already updates instantly and automatically with no JS involved:
  1. **Analog clock face.** `renderAnalogFace()` (script.js:217-224) reads `--clock-analog-day`/`--clock-analog-night` via `getComputedStyle(document.documentElement)` once and bakes the result as static SVG `fill` attributes rather than a live CSS reference. It's only re-invoked from `updateClock()` (the minute-boundary tick) and a few explicit UI actions (opening Clock Options, changing mode/scheme) — never from a theme change. The digital clock face is unaffected (pure CSS).
  2. **Weather widget's dynamic text color.** The `--wx-text-color` calculation (Build Log 10/11) reads `getComputedStyle(weatherWidgetEl).backgroundColor` (script.js:701), which resolves the theme-dependent `var(--surface)` — but that calculation only runs inside `renderWeatherSkin()`, which also isn't called on a theme change. If the gradient/cloud effects are active, the widget's actual background flips instantly (plain CSS), but the black/gray text-color decision stays computed against the *old* theme's base color until some unrelated setting change happens to trigger a re-render.
- [ ] Fix (covers both in one pass): in both places `data-theme` gets set — the manual theme-toggle click handler and the new auto-theme's `applyAutoTheme()` (both near the top of script.js) — also call `updateClock()` (redraws the analog face) and `renderWeatherSkin()` (recomputes the weather text color) right after. Also worth checking the Clock Options preview SVG (`clock-analog-preview-svg`) for the same staleness if that panel is left open across a theme change, though that's a much smaller edge case than the two live homepage widgets above.

### Live Condition Skin: detailed animation spec (from uploaded `weatherconditionskindetailedspec.md`)

The original "condition-specific base animations" idea was too vague to build from. This uploaded doc supplies the missing detail. Checked against everything already built for conflicts per the doc's own precedence rule — none found; it reuses/confirms the existing cloud-cover system and matches the DOM layer order already in place (cloud overlay appended before floating clouds, so clouds already paint on top). Not blocked on the WeatherAPI connection except for the very last bullet.

**Test/preview mode — corrected placement, overrides the uploaded doc:** the doc itself says to add "Preview Condition Skins" to the Weather Options long-press menu's "Visual Flourishes" section, but the user explicitly corrected this: it is **not** permanent UI and must **not** go in the Weather long-press menu. It belongs alongside the other controls in the existing `🧪 Testing Panel` (already clearly marked temporary, e.g. time override and cloud % slider) — it gets removed along with the rest of that panel once real API integration lands, not left behind as a permanent feature. Otherwise as specified: a picker covering all 10 states below, multi-select (needed for compound conditions like Thunderstorm + Hail), selected state(s) override the live skin regardless of actual conditions, clearly marked as preview, exit via tap-away/close/"Return to Live".

**Per-condition specs:**
- **Clear (Day):** warm gradient (yellow/light-blue) — this base color already exists via the sunrise/sunset keyframe system; new part is optional 4-6 thin light rays slowly rotating from a corner, one full rotation per several minutes, barely perceptible.
- **Clear (Night):** dark navy/deep blue gradient (also already exists via existing keyframes) + 15-25 small white dots with individually-randomized (not synced) opacity-pulse twinkle timing.
- **Partly Cloudy / Overcast:** no new logic — reuses the existing cloud-cover gray overlay + floating cloud emoji system as-is (Overcast is just naturally heavier via higher cloud %).
- **Light Rain:** 15-20 thin diagonal streaks (~15-20° off vertical), moderate speed/density, semi-transparent gray-blue, gray-blue gradient base.
- **Heavy Rain:** same streak system, 40-50 streaks, faster fall, darker/more saturated base, optional subtle bottom-edge "splash" flicker.
- **Thunderstorm:** Heavy Rain base + lightning flash (brief full-widget white/pale-blue-white overlay at 20-30% opacity, 100-150ms, randomized every ~8-20s). **Safety-critical:** one flash at a time, ≥8s minimum gap — avoid rapid/repeated flashing (photosensitivity).
- **Snow:** 20-30 small white flakes, slower fall than rain, gentle randomized (per-flake, not synced) horizontal sway, cool light gray/white base.
- **Fog/Mist:** soft low-contrast gray gradient + optional 2-3 large, very-low-opacity (10-15%) soft blurred drifting shapes — much larger/softer than the discrete cloud emojis, meant to read as haze not clouds.
- **Hail:** 25-35 discrete pellet shapes (not streaks — must read as distinct from rain), fast near-vertical fall (less wind drift than rain), **required bounce-on-impact** (small upward hop then settle/fade — this is hail's defining visual trait, not optional), cool gray base slightly darker than Heavy Rain.

**Compound conditions:** driven by actual API condition data (which layers are active), not a fixed enum of single states — layers stack additively rather than needing a hardcoded animation per combination. Example given: Thunderstorm + Hail = Heavy Rain base + hail pellets (with bounce) + lightning flash, all rendering simultaneously.

**Fixed layer stacking order (bottom to top):** sunrise/sunset gradient → cloud gray overlay → precipitation particles (rain/snow/hail, whichever active) → floating cloud emojis → lightning flash (topmost effect) → widget content (always topmost, always legible, no exceptions). This already matches the current DOM order for the two layers that exist today.

**General notes:** lightweight CSS/Canvas, mindful of battery on a page that may stay open/idle for long periods; reduce animation opacity/intensity rather than removing content if legibility suffers (the dynamic text-color system from Build Log 10/11 already handles most of this, but doesn't replace the need to keep animations themselves restrained); gradient still provides the base color with condition layers on top, not a replacement; state transitions should cross-fade over a few seconds when real conditions change, rather than snapping — explicitly a nice-to-have, not a hard requirement for the initial build.
...
- [ ] Root cause found and confirmed with an isolated repro (not just theorized): `randomizeCloud()` re-randomizes `animation-delay` (`(-Math.random() * duration) + 's'`) every time it runs — correct at creation, where the point is to stagger multiple clouds so they don't all start in visual lockstep, but wrong on respawn from the `animationiteration` handler. Reassigning a negative `animation-delay` on an animation that's already running makes the browser reinterpret "how far into the cycle am I right now," which can jump the element to an arbitrary point along its whole path, not back to the start — and if that recalculated position happens to land past the iteration's end, it immediately fires *another* `animationiteration`, which reruns the same broken logic, cascading several jumps in rapid succession before one happens to land near a plausible-looking spot. Reproduced directly in an isolated test page mimicking the exact same logic: a 1-second-duration animation fired 14 iteration events in 3.5 seconds (should be ~3–4), with computed `left` landing anywhere from `-40px` to `215px` on a 300px box — i.e. the "jumps all over, then settles somewhere visible" symptom exactly.
- [ ] Fix: stop touching `animationDelay` on respawn. Split `randomizeCloud()` so the random negative delay is only ever set once, at creation (for the initial stagger), and never reassigned from the `animationiteration` handler — respawn should only update `top`/`fontSize`/`animationDuration`/`--cloud-w`, which was already confirmed safe to change at the iteration boundary without a jump (that part of the Build Log 11 verification was correct; the delay reassignment was the missed variable).
...
- [ ] Condition-specific base animations for Live Condition Skin — the design spec that was missing here is now written (see "Live Condition Skin: detailed animation spec" below, outside this API-blocked cluster). Only the last piece — auto-selecting the active condition from real API data — is actually blocked on the API connection; the animations themselves and their manual preview mode are not, and can be built/tested entirely without it.
