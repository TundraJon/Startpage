...
### Grouping Up/Down — move a grouping's position within its category

- [ ] Per the user: no way today to move a grouping's own position — the only way a divider's place in the order changes is indirectly, by dragging tiles across it. Wants to select a grouping and have Up/Down buttons to move it directly.
