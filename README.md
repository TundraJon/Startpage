...
- [ ] **Design correction — full layer stack, back to front, as clarified by the user:** (1) white background, bottommost/base; (2) sky gradient color, layered on top of the white; (3) cloud cover, layered on top of the sky; (4) weather effects (rain/snow/etc., once built), layered on top of the cloud cover; (5) the actual data — text, numbers, icons — always the topmost layer, on top of everything else. Current implementation doesn't match this: the sky gradient is a faint ~18% tint sitting *behind* an essentially-opaque white card, and cloud cover/data aren't layered in this explicit order at all. Open concern from the user, not yet resolved: at night the sky goes near-black (`#020617`) and the current text colors are fixed dark values — that combination would be unreadable once the sky layer actually shows through at strength, and needs to be evaluated/solved (likely text colors becoming gradient-aware) once this is testable.
- [ ] Testing panel needs two more controls to evaluate the above: **(a)** a white-background toggle — ON renders a white layer with the gradient over it (adjustable per the slider below); OFF removes the white layer entirely, leaving just the raw environmental gradient colors full-strength. **(b)** an opacity slider for the gradient, active while the white background is ON, ranging from barely-there to fully dominant, so the right balance (and the resulting text readability at each point) can actually be seen and judged rather than guessed at.
- [ ] Candidate idea for the night-readability problem above (user explicitly floated this as a "spitballing," not a locked decision — evaluate via the testing panel once it exists, don't commit blind): a subtle text-stroke/outline around the text (and possibly the emoji) — either white or medium gray, whichever reads better in practice — to help characters stand out regardless of what's behind them, as a possible alternative/complement to making text color itself gradient-aware.
...
### Clock: timezone pill disappears in analog mode

- [ ] The EDT pill currently lives inside `.clock-digital`, so it disappears entirely whenever analog mode hides that whole block. Move it out of `.clock-digital` and place it under the date stack (`.clock-date`) instead, anchored to the bottom-right corner of the widget — visible in both digital and analog modes, out of the way of the clock face itself either way.
