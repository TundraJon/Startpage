...
- [x] **(1) decided:** only the one dragged tile gets un-checked — since drag/drop only ever moves one tile at a time, dropping can only ever apply to that single tile.
- [x] **New rule this settles: drag/drop disables entirely whenever 2+ tiles are selected**, per the user — not stated before. Needs its own guard: `armTileDragFromSelectMode`'s pointerdown trigger (and the existing `moveMode`-active-immediate-trigger branch alongside it) should refuse to arm/start a drag at all while `selectMode.selectedIds.size > 1`.
- [x] **(2) confirmed:** the movement-threshold-gating approach (reusing `armTileDragFromSelectMode`'s existing pattern instead of `moveMode`'s current immediate-trigger) is the right fix.

### Move Entry: touch-and-drag doesn't work at all on a real device (root-caused)

- [ ] **Reported:** on a real device, long-press correctly toggles select mode, but the touch-and-drag itself does nothing — neither the originally-selected tile nor any other tile in the category can be picked up and dragged. Worked fine "a few builds ago" (the old tile-menu-popup-triggered flow, Build 49-era).
- [ ] **Root-caused — a real CSS gap in Build 51's approach, not a logic bug (and not caught by this session's own testing, which only ever simulates mouse input, not real touch):** `.tile`'s base rule (styles.css:413) is `touch-action: manipulation` — this *permits* the browser's own native panning/scrolling for that element. It only becomes `touch-action: none` (fully JS-driven, no native gesture competing for the touch) via `.tile-grid.move-mode .tile` (styles.css:451-454) — and that rule only applies once `moveMode` is already active, i.e. **after** `enterMoveMode()` has already run.
  - `armTileDragFromSelectMode` (script.js) is specifically designed to watch pointer movement *before* deciding whether this is a drag — `enterMoveMode()` (and therefore `touch-action: none`) doesn't get applied until *after* movement has already crossed its 20px threshold. On a real touchscreen, that gap is exactly when the browser's own native scroll/pan handling (which `touch-action: manipulation` explicitly allows) can claim the gesture first — once that happens, the browser can stop delivering `pointermove` events to JS for it, so the threshold check in `armTileDragFromSelectMode` may never even get the chance to fire. `e.preventDefault()` on the initial `pointerdown` (added this session, for the separate native-link-drag issue) doesn't fix this — `touch-action`'s gesture-recognition disposition for a touch sequence is decided independently of a single event's `preventDefault()`.
  - This also explains why the old, `moveMode`-already-active branch (`if (moveMode && moveMode.grid === a.parentElement) startTileDrag(a, e);`) doesn't have this problem, and why it's specifically "the first drag" that's reported broken: by the time that branch runs, `moveMode` (and therefore `touch-action: none`) is already active from a *prior* successful drag — but since the first drag never successfully engages on a real device, `moveMode` never gets entered at all, so neither capability the user described ("touch-and-drag" and "drag/drop any other tile") ever gets a chance to work. One root cause explains both symptoms.
  - Consistent with why this session's own Playwright verification passed: it only ever simulates *mouse* input, which isn't subject to `touch-action` panning rules the way real touch input is — the exact gap that only shows up on a real device.
- [ ] **Proposed fix:** apply `touch-action: none` earlier — as soon as tile-select mode itself begins, not only once an actual drag has already started. Concretely, add a `.tile-grid.select-mode .tile { touch-action: none; }` rule (mirroring the existing `.move-mode` one) — `.select-mode` is already added to the grid at `enterSelectMode()` time (script.js) but currently has no CSS tied to it at all. That closes the gap for the *entire* select-mode session, including `armTileDragFromSelectMode`'s threshold-watching phase, matching the immunity the already-working `moveMode`-active branch has always had.

### Weather widget: clouds still reset every minute (root-caused) — snow/fog/hail don't

- [ ] **Reported:** clouds specifically still visibly reset every minute, contradicting Build 50's fix. Snow, fog, and hail do **not** reset — only clouds.
- [ ] **Root-caused — Build 50's own fix is what newly exposes this, not a leftover of the original bug:** Build 50 made `renderWeatherSkin()` run directly from the clock's per-minute tick, unconditionally, so the sunrise/sunset gradient keeps progressing even when `loadLiveWeather`'s fresh-cache branch is now correctly a no-op. But `renderWeatherSkin()` (script.js:3357) also owns the cloud layer, and its "preserve existing clouds" path (script.js:3410-3414, added in Build 12) re-appends every existing `.wx-skin-cloud` element on **every single call**, with a comment claiming "moving an existing node via appendChild doesn't restart or otherwise disturb its running CSS animation."
  - That claim looks incorrect, or at least not reliable across browsers: `.wx-skin-cloud`'s drift is a plain CSS `animation` (styles.css:783-797, `animation-name: wx-cloud-drift`, `infinite`), positioned via a fixed **negative** `animation-delay` set once at creation (`-Math.random() * duration`) to make it appear already mid-drift. If a browser treats *any* `appendChild` — even of a node that's already the last child, already present in the same parent — as reconnecting the element for animation-timing purposes, the negative delay gets re-evaluated relative to *that new connection moment* instead of continuing to progress from where it actually was, which would visually snap the cloud back to a fixed spot in its cycle. Build 12 called `renderWeatherSkin()` only occasionally (on settings changes) so this was apparently never caught; Build 50 calling it *every single minute, forever* makes a rare-looking glitch into an obvious, reliably-reproducing one.
  - This also explains why snow/fog/hail are unaffected: they render on `precipCanvas`, a `<canvas>` element whose animation state lives entirely in JS-tracked particle arrays and a `requestAnimationFrame` loop — not CSS. Re-appending a canvas element to reorder it (script.js:3394, done every call for the same stacking-order reasons as clouds) doesn't touch that JS state at all, so it's immune to whatever re-append is doing to the CSS-driven clouds.
- [ ] **Not a quick one-line fix — the current layering approach may need to change:** clouds only get re-appended every call because the whole stack (stars, a freshly-recreated overlay div, precip canvas) gets rebuilt/re-ordered every time, and DOM order (not `z-index`) is what currently establishes "gradient → stars → cloud overlay → precipitation → floating clouds → lightning flash." Actually avoiding touching clouds on calls where nothing about them changed likely means switching that stack to explicit `z-index` layering instead of relying on append order, so existing clouds never need to move in the DOM at all just to stay visually on top — worth designing properly rather than patching around the symptom.
