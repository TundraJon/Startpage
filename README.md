...
- [ ] **Also requested: pasting a CATEGORY into Home should make it a new top-level category** (same rank as News/Entertainment). This looks like it already works once the fix above lands — `moveCategorySubtree` (script.js) already converts `newParentId === 'home'` to `parentId: null`, which is exactly what makes a category top-level. No separate logic appears to be needed; worth confirming once the tap-to-select-Home fix is actually built, in case something else is blocking it.

### Sort Home category's own tiles (by usage, and possibly alphabetically)

- [ ] **Reported:** wants Home's own tiles sortable the same way Build 53/54 added for every other category (Alphabetical / Most Used / Last Used).
- [ ] **Open question, not decided:** the existing Sort UI lives inside the Edit Category dialog (`openEditCategory`), which Home can't use — Home isn't a `categoryTree` entry at all, so it has no `stripeColor` and its `.category-name` ("Home") is hardcoded in index.html, not editable through that dialog or through anything else today. (Correction: that name is separate from `.site-title`, the "Startpage name" — the text between the theme toggle and the settings/help icons — which Settings does rename; those are two different pieces of text, not one.) Sort itself would work identically for Home (`loadCategoryTiles('home')`/`saveCategoryTiles('home', ...)` already exists, same as any other category), but it needs a new place to live — a small dedicated control near Home's header (alongside the existing +/▲ buttons) is one option, but this hasn't been decided.
- [ ] **Also noted:** unlike other categories (a thin top stripe), Home's header already uses a *full* background color — currently hardcoded to `--home-header-bg: #1A5276` (styles.css), a blue picked as "my wife's favorite color." Per the user, the eventual Personalization work makes that full-background color changeable — **for Home only**. Every other category stays exactly as it is today, a thin top stripe, unchanged — Home is deliberately special/different by design, not a preview of where every category header is headed. Not a Sort blocker, just context for whenever Personalization is picked up.
