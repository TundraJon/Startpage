...
### Tile + category long-press multi-select: range-select, select-all/clear, scale/lift feedback, bottom-bar action menu

- [ ] **Requested:** extend the existing tile long-press-to-select (tap-to-toggle only today) with range-select ("subsequent long-presses select a range") and Select All/Clear, add the spec's "slight scale/lift" visual feedback on selection (doesn't exist today), and extend selection itself to categories as well as tiles — reversing the earlier "tiles only" call, since a real use case for selecting multiple categories came up. **Invert is dropped — not needed, per user.**
- [ ] **Current state:** `enterSelectMode`/`toggleTileSelected`/`selectMode` (script.js) handle one tile at a time via tap-toggle, scoped to a single grid (`selectMode.grid`) — no range tracking, no bulk select/clear, and no category involvement at all (`selectMode.selectedIds` is tile-only). `.tile-selected` (styles.css:1246-1266) only gets an outline + checkmark badge today — no transform/shadow.
- [ ] **Scale/lift feedback — decided design:** reuse `.tile.move-grabbed`'s existing `scale(1.05)` + drop-shadow (styles.css:417-420, the drag-to-reorder grab state) on `.tile-selected`, minus its `rotate(-3deg)` — selection should scale/lift only, no tilt.
- [ ] **Range-select mechanics — decided:** long-press one item to anchor the start of a range, long-press a second item to anchor the end — everything positionally between the two (in whatever list/grid they're both in) gets selected.
- [ ] **Category selection visual — decided:** a checkmark to the left of the category's title (not `.tile-selected`'s corner-badge treatment, which doesn't fit a full-width header row), colored using that category's own top/stripe color — i.e. the same `--stripe-color` value already driving its header today (set inline per category in `buildCategorySection`, script.js).
- [ ] **Category selection selects its whole subtree — decided:** selecting a category selects everything nested underneath it (subcategories and tiles alike) by default, matching the "moves as a unit" framing used elsewhere for category subtrees. Deselecting works the same way in reverse — deselecting a category deselects its subcategories underneath it too.
- [ ] **Select All / Clear scope — decided:** Select All applies only to tiles within a category, never to categories themselves. Clear applies to any selection, tiles or categories alike.
- [ ] **The tile-menu popup is retired — long-press now opens the bottom bar directly, decided:** long-pressing a tile or category no longer opens `#tile-menu-overlay` (today's popup with Rename/Move/Select/Delete). It goes straight into selection (selecting whatever was long-pressed) and shows a bottom action bar with 4 icons: ✂️ Cut, ✏️ Rename, 📋 Paste, 🗑️ Delete. This reuses/extends the existing `#select-action-bar` (index.html:516-519, currently just a status span, a Cancel button, and one Cut/Paste button doing double duty) rather than being a new component — Rename and Delete are new buttons added to it. `wireTileGrids`'s `attachLongPress(a, () => openTileMenu(a), ...)` (script.js) needs to call straight into (an extended) `enterSelectMode` instead of `openTileMenu`. **`#tile-menu-overlay` and its Rename/Delete logic (`tileMenuRenameBtn`, `tileMenuRemoveBtn`) get repointed to fire from the new bottom-bar buttons instead of being deleted outright** — the underlying rename/delete behavior stays, just its entry point moves.
  - **Move Entry (drag-to-reorder within a category) is explicitly unaffected — confirmed with user.** It stays exactly as built (Build Log 49): pick up a tile via its own separate flow and drag to reorder within the same category. This bottom bar's ✂️/📋 (Cut/Paste) is a different mechanism (moving between categories, or now potentially reordering too), not a replacement for the drag gesture itself.
  - **Select All / Clear icons — confirmed:** 🅰️ for Select All, 🆑 for Clear — both live on the same bottom bar as the other four.
    - Note Select All stays tiles-only per the earlier decision above — 🅰️ only ever selects tiles within the current category, never categories themselves.
  - **Rename scope — decided:** ✏️ is only active when exactly one item is selected (a single name field can't apply to more than one item at once) — disabled/inert otherwise.
  - **Category selection's action set — decided, differs from a tile's:** ✂️ Cut, 🔧 Edit, 📋 Paste, 🗑️ Delete — Edit replaces Rename and opens the same dialog as creating a category (name + color fields). **Dependency worth flagging:** that dialog (the "+ Category" overlay, Build Log 48) currently only has a name field — no color field exists yet, since the category color picker (Build Planner item 4) is still undecided/unbuilt. Edit's color field can't do anything real until that lands; until then Edit only changes the name.
    - **Edit's target when a subtree is selected — confirmed:** since selecting a category auto-includes its whole subtree (decided earlier above), Edit always acts on the *topmost* category of the current selection — the one actually long-pressed/tapped to anchor the selection — never on any of the auto-included descendants it dragged along. Per user's own examples: selecting a top-level category (auto-including all its subcategories) and pressing Edit changes only that top-level category's name/color, not any subcategory's; selecting "Subcategory B" (auto-including B's own children) and pressing Edit changes only B's name, not B's children's.
    - **Still open, not covered by the examples above:** what happens if two *separate* category subtrees are both selected at once (e.g. long-pressing Category A, then separately long-pressing Category C elsewhere) — there'd be two "topmost" selected categories with no single answer for which one Edit should target. Worth confirming when this is built; a simple resolution would be disabling 🔧 Edit whenever the selection spans more than one such root, mirroring Rename's exactly-one-tile rule.
  - **Icon order — confirmed:** ✂️ Cut, ✏️ Rename (🔧 Edit for a category), 📋 Paste, 🗑️ Delete — the emoji sequence, ending in Delete as intended.
- [ ] **Downstream:** once categories are genuinely selectable here, item 5's "Cut + Paste for whole category subtrees" entry (Build Planner) is unblocked — it was waiting on exactly this.
...
- [ ] **2. Long-press multi-select, extended to categories, with range-select, select-all/clear, and scale/lift selection feedback — promoted to the Build Queue.** See the Build Queue entry above for the full build-ready writeup. (Earlier logged here as tiles-only, since a category-selection use case hadn't come up yet — reversed per user, categories are back in scope.)
...
- [ ] **5. Cut + Paste (move only, not Copy — see precedence note above) for both tiles and whole category subtrees.** Tile move already exists two ways: single-tile drag (Move Entry) and multi-select batch move (`confirmMoveSelected`) — both already relocate-only, matching the decision here. Moving a *category* is new: needs to move an entire subtree (the category plus all its nested subcategories and tiles) to a new parent in one operation, reusing the same "select → navigate to destination → confirm" pattern multi-select tiles already use, once categories are real, selectable, movable objects (items 1 + 2 — item 2 now covers category selection too, per its Build Queue promotion above).
...
- [ ] **8. Rename — single exists, batch is new.** Single tile rename exists (`tileMenuRenameBtn` flow). Category rename doesn't exist (depends on item 1). Batch rename with pattern support (prefix/suffix/counter/find-replace) across multiple selected tiles or categories is entirely new — no pattern-rename logic anywhere in the codebase today.
- [ ] **9. Delete — single exists (with the standing easter-egg double-confirm), batch and soft-delete/trash don't.** `removeTile`/`tileMenuRemoveBtn` already handles single-tile delete with a confirm dialog. Batch delete for a multi-selection doesn't exist (today's multi-select only supports Move). Soft-delete/trash ("if feasible" per the doc) is a bigger, separate design — nothing about it decided.
