...
- [ ] **Confirmed root cause via live reproduction, not a crop/`object-fit` issue as the report's own wording suggested.** Uploaded a genuinely rectangular (300×150) test photo and inspected the actual stored/rendered pixels directly: the stored image is correctly a square 64×64 JPEG (the center-crop logic works exactly as intended — confirmed by sampling distinct left/right pixel colors from the source, both present and undistorted in the stored result). The bug is elsewhere: `#profile-btn-default-icon` (the person-silhouette `<svg>` in the header) never actually hides once a photo is set. `applyProfilePhoto()` (script.js) sets `profileBtnDefaultIcon.hidden = true`, but **`SVGElement`'s `.hidden` property assignment does not add the `hidden` *attribute* the way `HTMLElement`'s does** — confirmed directly: after setting `.hidden = true` on the svg, `hasAttribute('hidden')` returns `false` and its computed `display` stays `block`. Setting `.hidden` on the `<img>` counterpart (`profileBtnImg`) works completely correctly by contrast (`HTMLImageElement` reflects it properly) — this is specifically an SVG-element quirk, not a general problem with the toggling approach.
- [ ] **Net effect:** with the old SVG icon never actually disappearing, both it and the new photo `<img>` end up rendering *simultaneously* as siblings inside `#profile-btn`'s `display: flex; justify-content: center` row — sharing the button's 32px width between two flex items instead of the photo getting the full circle to itself, which is what reads as "padding" on the photo's left and right edges (it's genuinely being squeezed narrower by the still-visible SVG next to it, not letterboxed by `object-fit`).
- [ ] **Same bug almost certainly affects `#profile-photo-default-preview`** (the matching default-icon `<svg>` in Settings' own Profile Photo preview, toggled the same broken way) — confirmed directly via the same test: `.hidden = true` on it also leaves `hasAttribute('hidden')` false and `display: block`. Not yet checked whether it's visually as noticeable there (smaller/different layout context), but the underlying cause is identical and it should get the same fix.
- [ ] **Also worth noting:** this exact bug shipped past my own Build 63 verification because that test asserted `defaultIconHiddenAfterPick` by reading the element's `.hidden` *property* (which correctly reports `true`, since the assignment succeeds as a plain JS property — it just doesn't do anything to the DOM attribute or rendering for an SVG). The test checked the wrong signal; it needed to check actual visibility (computed `display`, or `hasAttribute`), not the property value.
- [ ] **The fix:** stop assigning `.hidden` directly on these two `<svg>` elements; use `setAttribute('hidden', '')` / `removeAttribute('hidden')` instead (reflects correctly regardless of element type), for both `profileBtnDefaultIcon` and the Settings preview's default icon. The `<img>` counterparts can keep using `.hidden` as-is, since that already works correctly for them.
...
- [ ] **11. Export/Import JSON backup.** Still doesn't exist. Serialize the full `categoryTree` + every `category-tiles-*` key into one JSON blob for export, plus a matching import/restore path with validation. Needs a UI decision — Settings menu is the natural home (doubly so now that Settings is a full-screen view, Build Log 63).
