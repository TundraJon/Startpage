...
- [ ] **User correction — the gray stripe is NOT a bug:** the current `border-top: 3px solid var(--stripe-color, var(--fg-muted))` (styles.css:181) falling back to a visible gray when unassigned is an **intentional placeholder** so the stripe's appearance can be previewed before the long-press "Reassign Stripe Color" control exists. It should stay exactly as-is for now. Only once that Phase 2 long-press control ships does "unassigned" change its default to a transparent line (still occupying the same 3px, just invisible) — not something to build yet.
- [ ] **Requested fix, verified precisely:** keep `border-top: 3px solid var(--stripe-color, var(--fg-muted))` (styles.css:181) completely unchanged. Reduce only `.category-header`'s top padding to compensate for that 3px, from `padding: 10px 14px;` to `padding: 6px 14px 10px;` (styles.css:182) — **not** a straight 10-3=7px; verified empirically that 6px is the value that makes the rendered heights match exactly (38px = 38px), a few pixels off from the naive arithmetic likely due to box-sizing/rounding. `.category-header--home` (styles.css:204-210) is untouched.
