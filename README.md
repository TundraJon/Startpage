...
- [ ] **Conflict/gap identified, resolved with user:** the spec's cross-category move (Section 3) is built around drilling through nested subcategories ("Games → RPG → Battle Maps") — but this site has no subcategory concept at all, just 4 flat categories (Home/News/Shopping/Entertainment). **User's decision (deferred to recommendation): build the flat version now.** Hovering a *different* category's header for ~0.5s during a drag highlights it as a drop target (no subcategory headers to reveal, since none exist — this matches the spec's own explicit fallback behavior in Section 5 for "a flat category with only individual tiles, no nesting"). Dropping appends the tile to the end of that category's tile list. The cascading multi-level drill-down (spec Sections 3's hover-to-expand-subcategories mechanic, and checklist items about 3-level-deep nesting) has nothing to attach to yet and gets revisited once subcategories are built by some future doc.
