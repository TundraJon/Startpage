...
### Live Condition Skin: darker cloud overlay for Heavy Rain / Thunderstorm, plus night variants
...
- [ ] **Scope widened by user:** also needs to account for night — Light Rain, Heavy Rain, Thunderstorm, and Snow should each have their own cloud-tint treatment at night, not just a fixed gray regardless of whether the sunrise/sunset gradient is showing bright noon or near-black midnight. Currently the overlay's tint is one constant color (`#4b5563`) with no time-of-day awareness at all.
- [ ] **UI, confirmed with user:** tunable means an actual slider (or a couple of sliders) in the Testing Panel, same pattern as the existing cloud%/opacity controls — not just "pick a number and hope it's right." Given day/night now multiplies the per-condition variants (4 conditions × day/night), a single "storm cloud darkness" multiplier slider that scales all of them at once is probably more practical than 8 separate sliders — proposing this rather than assuming, worth confirming when this gets built.
- [ ] **Open design question, not decided here:** should the night variants be computed as "the day value, just multiplied darker again" (simple, independent of the actual sky color), or should they derive from/blend with whatever `computeSkyColors(now)` is currently returning (so the cloud tint stays visually coherent with the actual twilight/night gradient showing at that moment, rather than being a separately-tuned dark gray that might clash)? The latter reuses data that already exists and would auto-adjust through twilight rather than hard-cutting at a threshold, but is more work than a flat multiplier. Flagging both options rather than picking one.
- [ ] Currently the cloud overlay's tint color is created with no per-condition or per-time color logic at all (`.weather-skin-overlay`'s `background`, script.js:866 — only its *opacity* varies, via `cloudPct / 2 / 100`, script.js:867). Needs branching there for at least: base (unchanged `#4b5563`), Light Rain, Heavy Rain (darker than Light Rain), Thunderstorm (darker than Heavy Rain — its base is already Heavy Rain per the existing spec, so it should darken *on top of* Heavy Rain's tint, not independently), Snow, each ×2 for day/night. Suggested starting points for the two already discussed (day only, pre-widening): Heavy Rain ≈ `#3c444f` (~20% darker than base), Thunderstorm ≈ `#2d333b` (~40% darker than base) — starting points only, to be tuned live via the new slider(s), not treated as final.
- [ ] **Cross-cutting detail, easy to miss:** `WX_CLOUD_TINT_RGB` (script.js:655, currently `[75, 85, 99]` — the same fixed `#4b5563`) feeds the dynamic text-color composite calc (script.js:898, Build Log 10/11/12) as a hardcoded stand-in for whatever the overlay's actual color is. Whatever tint ends up on screen for a given condition/time combination, the composite needs to use *that* value, not always the base gray, or the black/gray text-color decision will be computed against the wrong tint.
