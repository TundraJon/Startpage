...
### Reorg Tree Tool redesign — toolbar + selection + expand/collapse, per the user's mock-up

- [ ] The user shared a mock-up of a "Preference List Sections" editor and asked for the Reorg Tree Tool to look/function like it. Confirmed: everywhere the mock-up says "Section," read "Category" — **New Section → New Category**, **New Subsection → New Subcategory**, **Remove Section → Remove Category**, the list's own heading becomes "Categories" (or similar) instead of "Preference List Sections."
- [ ] **New selection model, doesn't exist today:** tapping a row selects it (single-select, highlighted) — today rows have no selected state at all, they're purely drag targets. The toolbar/footer actions below all act on whichever row is currently selected.
- [ ] **New top toolbar**, replacing the current plain header text, with four actions:
  - **Properties** — opens the existing Edit Category dialog for the selected category. **Decided: edits the live category right away**, not the tool's own draft/working-copy tree — so a rename (or Sort/Groupings change) made via Properties takes effect immediately, independent of whatever unsaved drag moves are pending in this Reorg session. Implementation note: since the tool's own working-copy tree (`reorgWorkingTree`) is a separate cloned object, a live rename via Properties needs to also patch that clone's `name` (or the Reorg list will keep showing the old name until Save/reopen) — everything else in the working copy (structure/order) stays untouched by this, since Properties never touches parentId/order.
  - **New Category** — same as today's "+ New Category" button: always top-level, unrelated to what's selected.
  - **New Subcategory** — new capability: creates a category as a child of whatever's currently selected. Needs a selection to be enabled (no selection = disabled, similar to how the select-action-bar already disables buttons with nothing selected).
  - **Remove Category** — new capability: deletes the selected category (and everything nested under it) from the tool's own working copy. **Decided: needs the same named-item + impact-count confirmation** every other delete in the app already uses (`openTileConfirm`/`tile-confirm-counts` — "Are you sure you want to remove the [Name] category (and everything in it)?" plus the tile/subcategory counts), reusing that existing dialog rather than building a new one.
- [ ] **New bottom row: Section Up / Section Down → "Category Up" / "Category Down"** — moves the selected category up/down among its own siblings only (same parent, same level), rewriting `order` values in the working copy. An alternative to dragging, not a replacement — the ☰ handle drag-to-reorder-and-renest stays exactly as built. No-ops (or disabled) at the top/bottom of its sibling group.
- [ ] **Expand/collapse arrows, per the user:** starts fully expanded (matches today's always-flattened list), but each category with children gets a collapse arrow to tuck its subtree away. New capability — today's list has no collapse state at all, `reorgFlattenTree()` always walks and renders everything.
- [ ] Home stays exactly as built: greyed out, first in the list, non-interactive for every one of these actions (not selectable, no Properties/Remove/Up/Down, can't be a New Subcategory's parent).
