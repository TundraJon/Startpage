...
- [ ] **Open question, not decided:** the existing Sort UI lives inside the Edit Category dialog (`openEditCategory`), which Home can't use — Home isn't a `categoryTree` entry at all (no `stripeColor`, and its display name comes from Settings, not a per-category rename dialog). Sort itself would work identically for Home (`loadCategoryTiles('home')`/`saveCategoryTiles('home', ...)` already exists, same as any other category), but it needs a new place to live — a small dedicated control near Home's header (alongside the existing +/▲ buttons) is one option, but this hasn't been decided.
