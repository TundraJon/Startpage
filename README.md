...
- [ ] **Not yet designed — to work out during the build:**
  - Range-select mechanics: what defines "a range" (index-based between two long-pressed tiles within a grid?), and whether/how a range can span tiles and categories together or only within one type.
  - Category selection's own visual language — categories are full-width header rows, not square tiles, so `.tile-selected`'s corner-checkmark-badge treatment doesn't map directly onto a header. Also: does selecting a category implicitly select its whole subtree (matching the "moves as a unit" framing used elsewhere for category subtrees), or just the header itself?
  - Select All/Invert/Clear scope: the current grid/category only, or everything visible/open at the time?
