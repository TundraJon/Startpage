...
- [x] **Editing placement — decided by the user.** Blurb editing belongs in the existing **Rename Tile dialog** (`#tile-rename-overlay`, reached via the ✏️ icon in the select action bar) — same tile-level surface as the name, not the Edit Category dialog. Blurb also belongs in the existing **Add Tile dialog** (`#add-tile-overlay`, Name + URL fields today) at creation time, per the user — both dialogs already exist in `index.html`, so this is adding one field (`blurb`) to each, not building new UI from scratch.
