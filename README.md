...
- [ ] Only the circular face itself should be white — the square `.clock-face` container around it is currently also forced white (`.clock-face[data-mode="analog"] { background: #fff }`), it should instead match the page background instead of overriding it to white. In dark theme, that square background should be a visible gray (not the page's near-black `--bg`, and not white) so the white circle reads as framed rather than blending in either direction.
- [ ] 24hr face specifically, dark theme only: the inner circle's shaded top half (currently a flat light gray, `#c9c9c9`, in all themes) should go to near-black instead, to keep the day/night contrast reading correctly against a dark-themed square background. Bottom half (white, "day") and the outer ring (white, unaffected by theme per the original spec) stay as they are.
- [ ] Numeral placement on both analog faces needs to move inward — the tick marks at the outer edge are currently overlapping/crowding the printed numbers, making them hard to read. Checked the code: this is a real, worse-than-cosmetic bug on the 24hr face specifically — the numerals (6/12/18/24) are placed at the exact same radius (41) as the inner edge of the tick marks, so they're colliding directly, not just crowded. The 12hr face's numerals (3/6/9/12) have some clearance from the ticks but could stand to move in too. User's estimate is ~10px inward on the real rendered size; exact amount to be tuned visually during the build rather than committed to blindly, but the 24hr zero-clearance collision should be fixed outright regardless of the final exact spacing.

### Digital clock overflow bug + black-white redesign

- [ ] The hour/minute digits (3.8rem, line-height 1.05) need **127.7px** of vertical space combined, but the clock face square is only **125.25px** tall (measured directly) — the numbers overflow the face's own box by ~11px on both top and bottom. This has been happening in *every* color scheme, but is invisible in red-black/blue-black/white-black because black text overflows onto the black `clock-inner` background behind it. It only becomes visible in black-white, where the face itself is white and the overflow spills onto bare black — reported by the user with a screenshot, reproduced and measured exactly. Root cause: the 3.8rem digit size was set before the face was later shrunk to a smaller square (78% height) to make room for the analog clock mode; nobody re-checked whether the digits still fit after that resize. Fix needs to either shrink the digit font size or grow the face height enough to fit both lines without overflow, verified for all 4 color schemes.
- [ ] **Design update for "black-white" specifically, per user sketch:** the face itself should stay black like every other scheme — only a white rounded box hugging just the hour+minute digits should be white, not the whole face. AM/PM and the EDT pill stay outside that white patch and render light-on-black (matching how they look in the other 3 schemes), not black-on-white. This needs to land together with the overflow fix above, since the white patch has to actually fit the digits within the same constrained face height.
