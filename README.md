...
### Personalization & Settings System — full spec, reconciled against what's already built

_The user supplied a full written spec for theme toggle/profile icon/Settings structure and a "Colors" personalization system, with the document's own explicit precedence rule: anything already decided/built supersedes it, stop and ask rather than silently pick on conflict. Three real conflicts surfaced against Build 60's already-shipped color system and were resolved directly with the user before logging this — **all three in favor of what's already built, not the document:**_
- _**Subcategory colors — doc superseded.** The document assumed every category, including subcategories, gets its own manually-picked stripe color. Build 60 deliberately built subcategories to auto-shade instead (a computed, progressively lighter version of their top-level ancestor's color via `lightenForDepth`, no picker of their own) — **kept as-is**, the document's assumption doesn't apply._
- _**Home's colorable elements — doc superseded.** The document wanted 3 independently-colorable Home elements (title bar / viewport / button backgrounds) from a ~20-color palette plus ~10 one-tap preset trios. Build 60 shipped 2 elements (title bar background + title text color) with the same 8-swatch palette Category Color uses, no presets — **kept as-is**; viewport/button colorization and the preset-trio picker are not being built._
- _**Centralized "Colors" Settings sub-screen — doc superseded.** The document wanted a dedicated full-screen Settings → Colors sub-screen (presets, individual Home pickers, and a scrollable all-categories list for reassigning stripe color) as the primary hub, plus a "Reassign Stripe Color" long-press quick-access point. Category Color already lives inline in the Edit Category dialog (reached via the Reorg Tree Tool's Properties button) and Home Color lives inline in Home Settings — **kept as-is**; no separate Colors screen is being built, and there's no standalone long-press quick-reassign shortcut anymore since Build 60 repointed category long-press to open the Reorg Tool instead._
- _Genuinely already satisfied, no resolution needed: the theme toggle (unchanged, no new work); the category stripe palette's ~8-10 color target (already 8); per-device persistence and no-confirmation-prompts-on-color-change (already how `localStorage`-backed Category/Home Color both work); and the document's own Section 4.5 flag (whether Home's custom colors should stay fixed regardless of the light/dark theme toggle) — already resolved exactly the way the document's own recommendation suggested, back in Build 60 (`--home-color-bg`/`--home-color-fg` are fixed, user-picked values, never auto-adjusted by theme state)._

- [ ] **Correction — the user caught a real gap in my first pass at this reconciliation:** I'd claimed "profile icon opens Settings, unchanged, no new work" was already fully satisfied. That's only half true. `#profile-btn` (index.html/script.js) is a static icon button wired to `openSettings` — real, built. But the document's actual Section 2 content (tap the icon → **native photo picker** → the browser holds a **local reference to a photo already on the user's device**, nothing uploaded, so the icon shows their own picture) **does not exist at all** — no `<input type="file">`, no stored image reference, nothing. The icon is just a fixed profile glyph today. **This is real, not-yet-built work.**
- [x] **Second pass, per the user's explicit request to re-check the document — two more findings, both now resolved:**
  - **Settings full-screen — confirmed, still wanted.** Section 3 wants Settings built as its own dedicated full/near-full-screen view (like the Reorg Tree Tool), instead of `#settings-overlay`'s current `.help-overlay` popup pattern (confirmed by reading the CSS — same dimmed-backdrop pattern every other dialog uses). The document's original reasoning ("given how much lives inside it, particularly Colors") is moot now that Colors isn't centralizing into Settings, but the user confirmed it independently anyway: **build it**, and per the user's own added reasoning, Settings has a couple more options coming that make the extra room worth it regardless of Colors. Not yet built — needs its own design pass (does it get the Reorg Tool's exact treatment — a separate `#settings-view` full-page div instead of an overlay; Cancel/Save or immediate-apply semantics for its existing Site Name/Theme/Weather/Organize sections).
  - **Grouping label text color — dropped, per the user: "I don't think we actually need that."** Not being built. The cross-reference in Section 4.3 (and the "Visual Grouping Headers" document it points to) is superseded by this decision — grouping labels stay their current fixed muted color, no picker, full stop.
- [x] **Profile photo picker — design resolved, not yet built:**
  - **Default state, per the user:** the current person-silhouette SVG (`#profile-btn`'s existing icon) stays the default for as long as no image has been assigned — it's not a placeholder to be replaced with something else, it's the permanent empty-state.
  - **Storage approach — resolved via a direct technical question from the user ("can an image be shrunk to basically an icon programmatically, by sampling pixels?"): yes, and this is the plan.** On pick (`<input type="file" accept="image/*">`), draw the source image into an offscreen `<canvas>`, center-cropped to a square, then scaled down to a small target size (e.g. 64×64, ~2x a ~32px icon for retina — canvas's own `drawImage` scaling *is* the pixel-sampling/downsampling the user was asking about, no custom algorithm needed). Export via `canvas.toDataURL('image/jpeg', ~0.85)` and store that small data URL directly in `localStorage` — shrinking before storing means a plain data URL is small enough (a few KB) that `localStorage` is sufficient; the earlier open question about needing IndexedDB is resolved, not needed. The circular shape comes from CSS (`border-radius: 50%` + `object-fit: cover` on an `<img>`), not a baked-in circular crop, so it stays crisp if the shape ever changes.
  - **Resolved — entry point, per the user:** tapping `#profile-btn` (top-right) always just opens Settings, same as today, whether or not a photo's been set — the image picker itself lives as its own entry inside the Settings menu, not a direct action on the icon. So changing the photo later means: open Settings → find the image-picker entry there → pick a new one.

_**Net result:** the Colors/subcategory/centralized-Colors-screen parts of this document are fully superseded by Build 60 — nothing new there. Two real gaps remain and need their own design passes before they're buildable: the profile photo picker, and converting Settings to a full-screen view (confirmed wanted, not yet built). Grouping label text color is a related but out-of-scope loose thread, not a decision from this document._

### Settings: move Weather API Key to the very bottom

- [ ] Per the user: `#settings-overlay`'s "Weather" section (`#weatherapi-key-input`, index.html) moves to the very bottom of the Settings menu — below "Organize" (currently the last section). The user's reasoning: it's a one-time-setup field, "only ever going to be referenced once and never again," so it shouldn't sit ahead of things referenced more regularly. Order becomes Site Name → Theme → Organize → Weather. Applies to whichever container Settings ends up in — this can land either before or after the Settings-full-screen-conversion entry in the Build Planner is actually built, doesn't depend on it.

### Design change: deleting a grouping should un-group its tiles to the top, not silently absorb them into the previous grouping

- [ ] Per the user (companion to, and the inverse of, the new-grouping-placement bug above): with "Free" holding 3 tiles and "Paid" holding 3 different tiles, deleting the "Paid" grouping currently leaves its 3 tiles right where they were in storage order — which now reads as if they belong to "Free" instead, since nothing marks the old boundary anymore. The user's own words: "it makes it appear that the tiles are now part of a group they were never intended to be part of in the first place." **Their proposed fix, which they want built:** move that grouping's own tiles up above every remaining grouping — into the category's ungrouped run at the top — instead of leaving them in place to be visually swallowed by whichever grouping used to precede them.
- [ ] **Confirmed current behavior, from reading the code:** `groupingDeleteBtn`'s handler (script.js) does exactly `tiles.filter(t => t.id !== dividerId)` — the divider entry is removed and every other entry, including that grouping's own tiles, keeps its exact array position. Nothing currently moves the orphaned tiles anywhere.
- [ ] **Not yet built.** The fix needs to: identify the deleted divider's own block (same run-boundary concept `moveGroupingBlock`/the new-grouping-placement fix above both use — everything from the divider up to the next divider or the end of the array), remove the divider itself, and splice that block's *tiles* (not the divider) to the front of the array — ahead of any other divider, live-grid DOM included, not just storage.
- [ ] **One open detail, flagged for a default at build time rather than asked about separately:** if the category already has some genuinely ungrouped tiles at the top (tiles that were never in any grouping — e.g. "GitHub" in the earlier screenshot), should the newly-orphaned tiles land *before* or *after* those pre-existing ungrouped ones? Default plan, unless corrected: append them after the existing ungrouped run (preserving the orphaned tiles' own relative order among themselves, and not disturbing the pre-existing ungrouped tiles' order), so genuinely-always-ungrouped tiles stay first and newly-orphaned ones land right after — but before the first remaining grouping.
...
- [ ] **Sharpened by the user's follow-up — the actual trigger is the divider boundary, not distance to any particular tile:** "Vibrating occurs on the group BELOW where the drag is taking place. If the dragging tile is even 1 pixel above the 'Free' group line, the Google tile vibrates. The instant I cross the full tile below the 'Free' line, the Wikipedia tile starts vibrating." So it's the group's *first tile* right after whichever divider the pointer is currently closest to — Google while still above the Free divider, Wikipedia once past Free's own row heading toward Not free — not "an unrelated tile far away" as originally guessed.
- [ ] **Not yet reproduced live** — no build authorized yet, so this remains a code-review-based theory, now refined against the user's more precise description, not yet confirmed against the real drag interaction.
- [ ] **Revised likely root cause, from reading the drag code (script.js):** the dragged tile is never taken out of normal grid flow during a drag — `.move-dragging` (styles.css) only disables its transition; there's no `position: absolute` or similar. It stays a real, live grid item the whole time, visually pinned to the cursor only via a CSS `transform` (which doesn't affect layout) — so its *actual* untransformed grid slot can move independently of where it visually appears to the user. `findNearestDropTarget` treats a divider's "nearest point on its own box" as a distance that collapses to vertical-only once the pointer's x is anywhere in the grid (always true) — meaning near a divider's row, the divider itself, and the first tile of the group right after it, can end up nearly tied as "nearest" candidate. `reflowWithinCurrentGrid` has no hysteresis on this decision, so as the pointer hovers near that boundary, the dragged tile's *real* DOM position likely keeps toggling between "before the divider" and "after the divider, before that first tile" on alternating pointermove events. Each toggle changes whether that first tile (Google, then later Wikipedia) is sharing a row with the dragged tile or not — which is what visibly shifts it left/right, even though the dragged tile itself looks stationary under the user's finger (its transform is masking its own real position flapping back and forth).
- [ ] **Not yet fixed** — needs to actually be reproduced and watched before committing to a specific fix. Candidate fixes: (a) add hysteresis/a dead-zone around a divider candidate specifically, so crossing near one doesn't flip-flop the reorder decision every pointermove; (b) scope `findNearestDropTarget`'s candidate search to just the dragged tile's own current run, so a divider one boundary away isn't even considered until the pointer is genuinely past it; (c) take the dragged tile out of normal grid flow while dragging (e.g. `position: absolute` sized/positioned to match, or a placeholder element left in its real slot) so its true DOM position toggling — however it's decided — never visibly perturbs a neighboring tile's own layout mid-drag. These aren't mutually exclusive; more than one may be needed.
...
- _**Move-only, no real Copy/duplicate.** The document's "Cut/Copy/Paste" clipboard model is scoped down: only Cut (move) is being built. Every "Copy" mention below is out of scope — a possible future add, not part of this entry._
- _**Icons stay emoji**, not Google Material Design icons as the document specified — matches the tile menu (✏️🖐️☑️🗑️), theme toggle (☀️🌙), weather, and `.category-collapse-btn` (▲) already throughout the app. No icon-font/SVG-sprite dependency being added._
- _Also note: **dark mode and favicon auto-fetch are already fully built** — the document lists both as requirements, but neither needs new work._
...
- [x] **2. Long-press multi-select, extended to categories, with range-select, select-all/clear, and scale/lift selection feedback — built (Build Log 50), but the *category* half of this is now superseded — kept here for history, not current state.** The tile-menu popup retirement and Move Entry's long-press-then-drag trigger are both still exactly as built and still current. Category select mode itself — the part of this item that extended long-press/range-select/checkmarks to categories — was fully removed in Build 60: a category's long-press now opens the Reorg Tree Tool directly instead (pre-selected, scrolled to it), which covers single-category rename/delete/move through its own Properties/Remove/drag. What's gone with it, not rebuilt anywhere: category range-select and category multi-select entirely (only one category at a time is ever actionable now, via the Reorg Tool).
- [x] **3. Create UI — built (Build Log 48).** Home's header is now sticky with a "+ Tile"/"+ Category" entry point (a "+ Grouping" option joined them in Build 60/61). Category creation currently sets `stripeColor: null`, same as always.
- [x] **4. Color picker for primary (top-level) categories — built (Build Log 60), not open anymore.** 8 default swatches + a native `<input type="color">` for a wider palette + a synced hex field, in the Edit Category dialog (reached via the Reorg Tree Tool's Properties button). Scoped to primary/top-level categories only, exactly as this item specified. **Subcategory color — also resolved, not "to be settled" as this entry used to say:** subcategories don't get their own picker at all — they automatically render a progressively lighter shade of their top-level ancestor's color (`lightenForDepth`: +12% HSL lightness per nesting level, clamped at 92%), computed via `primaryAncestorAndDepth`. This exact mechanism was also explicitly re-confirmed as the intended one (not switched to per-subcategory manual picking) when reconciling the later Personalization & Settings System spec, now in the Build Queue.
- [x] **5. Cut + Paste (move only, not Copy — see precedence note above) — built for tiles (Build Log 50), but the *category-subtree* half was later removed (Build 60), not rebuilt.** `moveCategorySubtree` (whole-subtree Cut+Paste) depended on category select mode, which no longer exists — this was an accepted tradeoff when category long-press was repointed to the Reorg Tree Tool instead (item 2 above). A single category's move is still fully covered, just via the Reorg Tool's own drag-to-reorder/re-nest rather than Cut+Paste. Tile Cut+Paste is untouched and still works exactly as built.
- [x] **6. Drag-and-drop reordering of tiles within the same category — already built, no new work.** This is exactly Move Entry's same-grid reorder path (`reflowWithinCurrentGrid`, nearest-tile-center detection, live visual feedback via `move-dragging`/`move-grabbed` classes) — confirmed it already satisfies this requirement as written. (There's a real, separate, currently-unfixed bug in this same reorder path — see the drag-jitter entry in the Build Queue — but that doesn't change that the *feature itself* is built.)
- [ ] **7. Search across all tiles, with auto-navigate + scroll-into-view + glowing highlight.** Still nothing like this exists — the only search bar in the app is the external web-search form (`#search-form`, submits to Google/Bing). Needs: an index over every category's stored tiles (DOM-driven or storage-key-driven, same open question the "Reports" idea below has), a results UI showing title + category path, and on selecting a result: computing that tile's category chain and calling `openCategoryPath` on it, then `scrollIntoView` plus a new CSS pulse-glow animation (light blue border, ~1.5–2s fade) — no existing animation to reuse, would be new.
- [ ] **8. Rename — single exists for both tiles and categories, batch is still new.** Single tile rename (bottom bar's ✏️, `openTileRenameFor`) still works as before. Single category rename now happens through the Edit Category dialog's Name field (reached via Reorg Tree Tool → Properties, not a standalone "🔧 Edit" button anymore — that went away with category select mode, item 2 above) — the *capability* is unchanged, just its entry point. Batch rename with pattern support (prefix/suffix/counter/find-replace) across multiple selected items is still entirely new — no pattern-rename logic anywhere in the codebase.
- [ ] **9. Delete — tile delete (single and batch) still exists; category delete is now single-only, batch is gone.** The select-action-bar's 🗑️ (`deleteSelected`) still handles any tile selection, one or many, easter-egg double-confirm intact. Category delete now happens one at a time via the Reorg Tree Tool's own "Remove Category" button (impact-count confirmation, same friction model) — batch/multi-category delete no longer exists, since it also depended on category select mode (item 2). Soft-delete/trash ("if feasible" per the doc) is still undesigned, nothing decided.
- [ ] **11. Export/Import JSON backup.** Still doesn't exist. Serialize the full `categoryTree` + every `category-tiles-*` key into one JSON blob for export, plus a matching import/restore path with validation. Needs a UI decision — Settings menu is the natural home (doubly so now that Settings is slated to become its own full-screen view, per the Personalization spec in the Build Queue).
- [ ] **12. Undo for the last major action (move/delete/reorder).** Still doesn't exist anywhere — every mutation writes straight to localStorage with no history. Needs a small undo-stack design (what's captured, how far back, cleared when) before it's buildable.
- [ ] **13. UX polish items already substantially covered:** large touch targets, dark mode, smooth animations (Move Entry, category expand/collapse, Reorg Tool drag) all already lean this direction, nothing new needed there. Haptic feedback (`navigator.vibrate`, where supported) remains the one genuinely new, small addition in this group.
- [ ] **Open, not yet decided:** exact shape of "current location" for create (item 3) when nothing is open; the undo stack's design (item 12); soft-delete/trash's design (item 9); whether category range-select/batch-select and batch category Cut+Paste/delete (all dropped in Build 60, per items 2/5/9 above) are wanted back in some form, now that the Reorg Tree Tool exists as their replacement — not raised again since Build 60, worth confirming is still the accepted tradeoff rather than assuming so indefinitely.
