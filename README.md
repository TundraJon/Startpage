...
- [ ] Per the user, with a screenshot (Sample Category A: GitHub ungrouped, then "Free" — Google/Translate — then "Not free" — Wikipedia alone): while dragging the Translate tile around near the Free group, the Wikipedia tile (in the separate Not free group, further down) "vibrated left/right constantly" the whole time — without the drag ever getting near Not free itself.
- [ ] **Not yet reproduced live** — no build authorized yet, so this is a code-review-based theory only, not confirmed against the real drag interaction.
- [ ] **Likely root cause, from reading the drag code (script.js):** the dragged tile is never taken out of normal grid flow during a drag — `.move-dragging` (styles.css) only disables its transition; there's no `position: absolute` or similar. It stays a real, live grid item the whole time, visually offset on top via a CSS `transform`, while `reflowWithinCurrentGrid`/`findNearestDropTarget` actually reparent it in the DOM (`.after()`/`.before()`) as the pointer moves. Since one whole category's grid (all its groupings included) is a single continuous CSS grid — dividers are just full-width break items inside it, not separate grids — reparenting the dragged tile anywhere near the top re-flows every row below it, including tiles in a completely different grouping further down (here, Wikipedia's row). `findNearestDropTarget` also searches **every** tile/divider in the whole grid, not just the ones in the dragged tile's own current run between dividers, so nothing scopes the reorder candidate search to "this group only" either.
- [ ] **Likely trigger for the specific "vibrating" symmetry, not just a one-time shift:** `reflowWithinCurrentGrid`'s own reorder decision ("is the pointer closer to a candidate's slot than to the tile's own current slot") has no hysteresis — if two candidates end up close to equally near the pointer (plausible here, since every reflow it causes nudges the grid's row-wrapping, which can nudge those very distances), the tile can flip back and forth between two DOM positions on alternating pointermove events, each flip re-triggering the same downstream row-wrap shift. A tile sitting alone in its own row two groups down (Wikipedia) would visibly hop left/right exactly if that shift toggles which column it wraps into.
- [ ] **Not yet fixed** — needs to actually be reproduced and watched (ideally with the grid's row/column boundaries visible) before committing to a specific fix; the two candidate fixes suggested by the theory above are (a) scoping `findNearestDropTarget`'s candidate search to just the dragged tile's own current run (between its enclosing dividers, or the grid's start/end) so a drag can never even consider a tile in a different grouping as "nearest," and (b) taking the dragged tile out of normal grid flow while dragging (so reparenting it doesn't itself reflow anything until the drag ends) — either might be sufficient alone, or both might be needed together.
