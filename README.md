...
- [ ] **User wants this tunable via the Testing Panel, not a one-off hardcoded guess** — same pattern as the existing "Cloud Brightness Formula" sliders (`WX_CLOUD_TUNABLES` + `test-cloud-*-slider`, Build Log 13). A `WX_FOG_TUNABLES` object (opacity, and likely blob size/count/speed) with a matching set of Testing Panel sliders, each writing straight into the tunable and re-rendering immediately, plus inclusion in the existing "Reset to defaults" button — so the right values can be dialed in live against the real widget instead of guessed blind.
...
- [ ] **Random bounce direction, 30° left through 30° right of straight up:** at the moment a stone transitions from `fall` to `bounce` (script.js:970, where `bounceT`/`bounceFromY` are already reset), also roll a random angle from -30° to +30° off vertical (0° = straight back up) and store it on the particle. During the bounce, add a horizontal displacement alongside the existing vertical hop — driven off the same angle, so a stone can shoot off to either side or bounce straight back, matching what's asked. Tying the horizontal distance to `hop * tan(angle)` (rather than an independent random range) keeps it physically sensible — a bigger bounce also drifts further sideways, a small one barely moves horizontally — and needs no extra tuning constant.
- [ ] **Random bounce height, per the user's precise spec:** ~75% of stones keep today's bounce height (the current fixed `8`) unchanged; the other ~25% bounce higher, by a random extra amount **between 1% and 100%** of the base height (i.e. that quarter's multiplier is `1 + (0.01 to 1.00)`, not a flat 100% bonus for all of them). Rolled once per bounce, same moment as the angle above, and reflected in both the vertical hop and (per the point above) the horizontal drift together.

- [ ] **Current behavior:** `stepConditionSkin`'s thunderstorm block (script.js, `if (c.has('thunderstorm'))`) fires a single full-widget screen flash (`.weather-skin-flash`'s opacity, `flashState`) lasting 100–150ms, then waits 8–20s before the next one — always a single isolated pulse, never back-to-back.
- [ ] **Double-flash request:** occasionally, two flashes should fire in quick succession — one immediately after the other — rather than always a single pulse. Implementation direction: when a flash triggers, roll a chance (e.g. ~30–35%) to schedule a second one shortly after the first ends (a short gap, roughly 80–180ms) before falling back to the normal long 8–20s gap; needs one more piece of state on `flashState` to track a pending second flash distinct from the next-normal-flash schedule.
- [ ] **Random lightning bolt request:** on each flash event, up to a 10% chance (independent of the double-flash roll) of also drawing an actual bolt graphic somewhere on the widget — randomly placed and randomly shaped each time, not a fixed image. Implementation direction: procedurally generate a jagged bolt path each time it fires (a zigzag random-walk from a random point along the top edge down to a random depth, optionally with one small fork/branch), draw it on the existing `precipCanvas` with a bright yellow-white stroke (a little glow via `shadowBlur` reads well) synced to the same brief on-screen window as its flash, then discard it — a fresh random shape next time, never the same bolt twice.
- [ ] Both effects only apply during `thunderstorm`, layered on top of the existing single-flash mechanism rather than replacing it — most flashes should stay as they are now (single, no bolt); the double-flash and the bolt are each occasional variations on top.
