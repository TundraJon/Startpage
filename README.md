...
- [ ] **Design correction — the gradient relationship is backwards from what was built.** Current implementation: the sky gradient is a faint tint (~18% opacity) sitting *behind* an essentially-opaque white card. Intended relationship: the sky color should be the *dominant* background; the white card (if present at all) is the adjustable layer, not the other way around. Open concern from the user, not yet resolved: at night the sky goes near-black (`#020617`) and the current text colors are fixed dark values — that combination would be unreadable once the gradient actually dominates, and needs to be evaluated/solved (likely text colors becoming gradient-aware) once this is testable.
