...
- [ ] **Also noted:** unlike other categories (a thin top stripe), Home's header already uses a *full* background color — currently hardcoded to `--home-header-bg: #1A5276` (styles.css), a blue picked as "my wife's favorite color." Per the user, the eventual Personalization work will make every category header's full background recolorable, not just a thin stripe — generalizing Home's existing full-background treatment to every category, not introducing it fresh. Not a Sort blocker, just context for whenever Personalization is picked up.
