...
- [ ] **Build order note:** the placeholder test content above is now built, so Move Entry's cross-category cascading hover-to-expand builds for real against Category C's actual 2-level nesting, not the flat-only fallback originally planned as a stopgap.
