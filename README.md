...
- [ ] Timezone pill (currently "EDT"): remove from next to the city/state on the weather widget's top row, and instead move it to the clock widget — centered underneath the displayed time — using the clock's currently-selected color scheme (e.g. red-on-black) instead of the weather widget's neutral pill styling.
