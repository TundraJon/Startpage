...
### Thunderstorm: occasional double-flash, and a randomly-generated lightning bolt
...
- [ ] **Random lightning bolt request:** occasionally (independent chance from the double-flash, e.g. tied to some fraction of flash events), draw an actual bolt graphic somewhere on the widget — randomly placed and randomly shaped each time, not a fixed image. Implementation direction: procedurally generate a jagged bolt path each time it fires (a zigzag random-walk from a random point along the top edge down to a random depth, optionally with one small fork/branch), draw it on the existing `precipCanvas` with a bright yellow-white stroke (a little glow via `shadowBlur` reads well) synced to the same brief on-screen window as its flash, then discard it — a fresh random shape next time, never the same bolt twice.
