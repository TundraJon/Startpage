...
### Tile + category long-press multi-select: range-select, select-all/invert/clear, scale/lift feedback

- [ ] **Requested:** extend the existing tile long-press-to-select (tap-to-toggle only today) with range-select ("subsequent long-presses select a range") and Select All/Invert/Clear, add the spec's "slight scale/lift" visual feedback on selection (doesn't exist today), and extend selection itself to categories as well as tiles — reversing the earlier "tiles only" call, since a real use case for selecting multiple categories came up.
- [ ] **Current state:** `enterSelectMode`/`toggleTileSelected`/`selectMode` (script.js) handle one tile at a time via tap-toggle, scoped to a single grid (`selectMode.grid`) — no range tracking, no bulk select/invert/clear, and no category involvement at all (`selectMode.selectedIds` is tile-only). `.tile-selected` (styles.css:1246-1266) only gets an outline + checkmark badge today — no transform/shadow.
...
- [ ] **Select All/Invert/Clear scope — decided:** Select All applies only to tiles within a category, never to categories themselves. (Whether Invert/Clear follow the same tiles-only scope, or also apply to a category selection, wasn't specified — worth confirming when this is actually built, since only "Select all" was called out explicitly.)
- [ ] **Still open:** does selecting a category implicitly select its whole subtree (matching the "moves as a unit" framing used elsewhere for category subtrees), or just the header itself? Not addressed by the decisions above.
...
- [ ] **2. Long-press multi-select, extended to categories, with range-select, select-all/invert/clear, and scale/lift selection feedback — promoted to the Build Queue.** See the Build Queue entry above for the full build-ready writeup. (Earlier logged here as tiles-only, since a category-selection use case hadn't come up yet — reversed per user, categories are back in scope.)
