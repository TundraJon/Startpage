...
- [ ] Weather widget: top row (wind/visibility/cloud extras, and location/timezone pill) should be 1.2x its current font size, to match the sizing of the other small text/icon rows.
...
- [ ] Weather widget: remove the timezone pill (currently "EDT") from next to the city/state on the top row — it's taking space better used for the location text.
