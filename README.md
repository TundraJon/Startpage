...
### Move Entry: drag/drop restricted to same-category reorder only; cross-category moves go through Select

- [ ] **Requested:** dragging a tile between categories still doesn't reliably flow to Home — rather than keep chasing that, only allow drag/drop to reorder tiles within their own category. Cross-category moves should go exclusively through the existing Select (Cut/Paste) flow.
- [ ] **Root-caused a real gap this decision surfaces — Select currently can't target Home as a destination at all:** `confirmMoveSelected` and `updateSelectActionBar` (script.js) both derive the paste destination purely from `openPath[openPath.length - 1]`, and `confirmMoveSelected` returns immediately if `openPath.length === 0`. Home is deliberately never part of `openPath` (it's special-cased, always visible, no "open" state) — so today, Select can *only* target a real navigated-into category, never Home. If drag/drop stops covering cross-category moves entirely, this gap would mean **Home becomes completely unreachable as a move destination** — worth fixing as a required part of this change, not a separate follow-up, since otherwise there'd be no way at all to move an existing tile into Home.
  - **Fix — reuse the same convention `currentLocationId()` already established (Build 48):** treat "nothing open" as "Home" for destination purposes too, instead of disabling Paste. Concretely: `destId = openPath.length > 0 ? openPath[openPath.length - 1] : 'home'` in both functions (dropping the `!destId` disabled-state and the "choose a destination" placeholder message, since a destination — Home by default — is now always resolvable).
- [ ] **What gets removed from Move Entry (drag), since cross-category is going away entirely:**
  - `handleHeaderHover`/`clearHeaderHover`, `HOVER_SETTLE_MS`, and `dragInfo.crossTargetId`/`lastHeaderEl`/`hoverSettleTimer` — the whole hover-a-different-category's-header-to-cascade-it-open mechanic. A drag no longer needs to react to anything outside its own source category.
  - `enterGrid` and the `dragInfo.currentGrid` vs. `dragInfo.grid` distinction — with only one grid ever in play, there's nothing to "enter." `processDragPosition` simplifies to always calling `reflowWithinCurrentGrid` against `dragInfo.grid`, regardless of what's actually under the pointer.
  - `finishTileDrag`'s `crossTargetId`/`liveCategoryId` branches — collapse down to just the existing same-category reorder-and-save path (today's final `else` branch).
  - The Build 46 "pinned chain" logic in `renderOpenPath` (the `pinnedGrid`/`pinnedCategoryId`/`pinnedChain` block, added to stop a dragged tile's category from collapsing out from under it while hovering elsewhere) — becomes dead code once dragging can no longer trigger navigation to a different category at all, so there's nothing left for it to protect against.
...
- [ ] **2. Long-press multi-select on tiles: range-select, select-all/invert/clear, and scale/lift selection feedback.** Tile long-press-to-select already exists (`enterSelectMode`/`toggleTileSelected`/`selectMode`, script.js) but is tap-to-toggle only, one grid at a time — no range-select ("subsequent long-presses select ranges"), no Select All/Invert/Clear. Scoped to tiles only — extending selection to categories themselves doesn't make sense (per user) and isn't part of this item.
  - **Scale/lift feedback — decided design:** the spec's "slight scale/lift animation" doesn't exist on `.tile-selected` today (styles.css:1246-1266, outline + checkmark badge only, no transform/shadow). It already exists elsewhere though — `.tile.move-grabbed` (styles.css:417-420, the drag-to-reorder grab state) does `transform: rotate(-3deg) scale(1.05)` plus a drop shadow. Per user: reuse that same scale(1.05) + box-shadow treatment on `.tile-selected`, minus the rotate — selection should scale/lift only, no tilt.
...
- [ ] **5. Cut + Paste (move only, not Copy — see precedence note above) for both tiles and whole category subtrees.** Tile move already exists two ways: single-tile drag (Move Entry) and multi-select batch move (`confirmMoveSelected`) — both already relocate-only, matching the decision here. Moving a *category* is new: needs to move an entire subtree (the category plus all its nested subcategories and tiles) to a new parent in one operation, reusing the same "select → navigate to destination → confirm" pattern multi-select tiles already use, once categories are real, movable objects (item 1). Selection mechanism for categories still needs its own design, since long-press multi-select isn't being extended to categories.
