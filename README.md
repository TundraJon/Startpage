...
- [ ] **Flash interval — switch to intensity-tiered timing, dropping the double-flash roll entirely.** Replace the current flat 8–20s gap + ~32% chance of a quick second flash with three intensity tiers: light 10–15s between flashes, medium 5–10s, heavy 2–8s. No separate "double-flash" scheduling needed — the new flash-duration flicker (next point) organically produces a similar "multiple pulses" look within one flash.
...
- [ ] **Storm intensity source — always "medium" from live data, no attempt to infer from condition codes.** WeatherAPI's thunder-related codes (patchy-light vs. moderate-or-heavy "...with thunder") describe rain intensity, not actual lightning/storm intensity, so deriving light/heavy from them (originally proposed) was rejected as not a genuine signal. Live thunderstorm conditions should just default intensity to `medium` uniformly. The Testing Panel keeps a light/medium/heavy override (radio buttons, mutually exclusive) for manual preview of all three tiers, since live data can only ever produce the one default.
- [ ] **Implementation note (from the prior build attempt, reverted before commit):** this needs `weatherState.thunderIntensity` (set to `'medium'` whenever `mapConditionCode()` maps to `'thunderstorm'`, nothing more elaborate), a `weatherTestState.thunderIntensityOverride` field, a `getEffectiveThunderIntensity()` accessor mirroring the existing `getEffectiveSkyTime()`/`getEffectiveCloudPct()` pattern, and three new radio buttons in the Testing Panel's thunderstorm row wired the same way the condition-skin checkboxes are, including reset-to-defaults handling.
