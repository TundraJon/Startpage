...
- [ ] **The tile-menu popup is retired — long-press now opens the bottom bar directly, decided:** long-pressing a tile or category no longer opens `#tile-menu-overlay` (today's popup with Rename/Move/Select/Delete). It goes straight into selection (selecting whatever was long-pressed) and shows a bottom action bar with 4 icons: ✂️ Cut, ✏️ Rename, 📋 Paste, 🗑️ Delete. This reuses/extends the existing `#select-action-bar` (index.html:516-519, currently just a status span, a Cancel button, and one Cut/Paste button doing double duty) rather than being a new component — Rename and Delete are new buttons added to it. `wireTileGrids`'s `attachLongPress(a, () => openTileMenu(a), ...)` (script.js) needs to call straight into (an extended) `enterSelectMode` instead of `openTileMenu`. **`#tile-menu-overlay` and its Rename/Delete logic (`tileMenuRenameBtn`, `tileMenuRemoveBtn`) get repointed to fire from the new bottom-bar buttons instead of being deleted outright** — the underlying rename/delete behavior stays, just its entry point moves.
  ...
  - **Select All / Clear icons — decided:** 🅰️ for Select All, 🆑 for Clear — both live on the same bottom bar as the other four. (User offered ➕ as an alternative to 🅰️ for Select All; going with 🅰️ since ➕ is already this app's "add new" glyph — `#create-btn`'s literal `+` — and reusing it here for a completely different action risks the exact mis-tap confusion this whole redesign is trying to avoid. Flagging the substitution in case ➕ was actually preferred.)
  ...
  - **Category selection's action set — decided, differs from a tile's:** ✂️ Cut, 🔧 Edit, 📋 Paste, 🗑️ Delete — Edit replaces Rename and opens the same dialog as creating a category (name + color fields). **Dependency worth flagging:** that dialog (`#create-category`-style overlay, Build Log 48) currently only has a name field — no color field exists yet, since the category color picker (Build Planner item 4) is still undecided/unbuilt. Edit's color field can't do anything real until that lands; until then Edit would only be able to change the name. By the same single-item logic as Rename above, Edit is presumably also only active with exactly one category selected — not stated explicitly, but the identical one-name-field problem applies, so flagging this as inferred rather than confirmed.
  - **Icon order — decided:** going with the literal emoji sequence given, ✂️ Cut, ✏️ Rename (🔧 Edit for a category), 📋 Paste, 🗑️ Delete — this is what was formatted with actual icons, as opposed to the prose list a few messages earlier ("cut, rename, delete, paste") which put Delete third instead of last; flagging the substitution in case the prose order was actually intended. This order also happens to serve the stated safety goal reasonably well — 🗑️ Delete sits alone at the end rather than sandwiched between two frequently-tapped actions, so a slip only risks landing on it from 📋 Paste, not from ✂️ Cut.
