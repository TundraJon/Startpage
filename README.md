...
- [ ] **Not fixed — flagging options, since this is an architectural gap Build 42 didn't anticit, not a one-line patch:**
  - Give the dragged tile a true escape hatch for the duration of the drag — reparent it into a persistent top-level layer (e.g. directly under `<body>` or `#categories`, `position: fixed`) so no category's `hidden` state can ever affect it, then reparent it back into a real grid only at drop time. Most robust, but the biggest change — introduces a "ghost" drag element separate from the category tree.
  - Never let `openCategoryPath` collapse whichever category currently contains `dragInfo.tileEl` while a drag is active — keep it visible as a standing exception to the single-open rule for the duration of the drag. Smaller change, but means two things can be visibly open at once during a drag (the tile's current location plus wherever hover navigated), which cuts against the whole point of the global-exclusive redesign.
  - Proactively call `enterGrid` to move the tile into whatever grid *is* currently visible whenever its current one is about to collapse, so it's never left behind in a hidden subtree. Avoids a new architecture, but needs a rule for *where* to move it each time, and could look like the tile is randomly jumping grids as the user hovers around.
