...
- [x] **"+ New Category" placement — decided:** the reorg tool gets its own "+ New Category" entry, per the spec, in addition to the existing global "+" button in the Home header — both stay, not an either/or.
- [x] **Home's position — decided:** Home appears in the tree but greyed out — not selectable/draggable/renameable for any function this tool offers, since Home itself can't be moved or renamed at all. Every existing top-level category (News, Shopping, Entertainment, etc.) remains fully reorderable/re-nestable among themselves, effectively as children directly under the greyed-out Home row. Today Home isn't part of `categoryTree` at all — it's hardcoded as the first section in index.html and never participates in any reorder/drag mechanics — so showing it here still means rendering a synthetic, always-first, greyed-out, non-interactive row.
