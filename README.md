...
No conflicts with existing implementation — the clock currently has no long-press menu at all (hardcoded digital/red-black/12hr), so nothing here contradicts what's built. Checked specifically for a short/long date toggle per the doc's flag-before-removing instruction — none exists; the date was already reverted to abbreviated-only (no toggle) during the earlier square-face work, for the same space reasons this doc cites, so that requirement is already satisfied as-is.

- [ ] Long-press menu on the clock widget (doesn't exist yet — currently only the weather widget has one): Digital/Analog selector, each option showing a live mini-preview before applying. Selecting Digital cascades open a submenu of the 4 color schemes (red/black, blue/black, black/white, white/black), each with its own live mini-preview of sample numbers. Selecting Analog shows a live preview of the face reflecting the current 12hr/24hr setting.
- [ ] 12hr/24hr toggle, shared between modes: in digital, adds/removes the AM/PM indicator; in analog, switches between the two face designs below. Persist per-device like the weather settings.
- [ ] Analog mode — 12hr face (not built yet, digital-only currently): white/light background, hour + minute hands, tick marks at all 12 positions, only 3/6/9/12 numerals printed, no shading anywhere.
- [ ] Analog mode — 24hr face (not built yet): two concentric zones. Outer ring stays uniformly white across its whole circumference, holds the 24 tick marks and only the 6/12/18/24 numerals. Inner circle (where the hands pivot) splits horizontally along the 6–18 axis (not the 12–24 axis) — top half (nearer 12/24) shaded gray, bottom half (nearer 6) white. Hour hand completes one rotation per 24 hours (half speed vs. a normal clock); minute hand behaves normally (one rotation per 60 min). Confirm this orientation against the Chicago Lighthouse reference image before finalizing, per the spec's own note.
- [ ] Digital mode layout is already correct per the current build (face left, 3-line date stack right, hour on top/minute on bottom, AM/PM small top-left in 12hr only) — no change needed, spec just restates what's already there.
