...
- [x] **Dropdown row content — decided by the user:** full category chain, then the tile name at the end — e.g. `Games > Computer > Role Play _ Free > Wolfenstein`. Note this example includes a *grouping* name ("Free," a Visual Grouping Header divider inside the "Role Play" category), not just category nesting — the user's own example marks the grouping segment with a leading `_` (underscore), distinct from `>` between real category-chain levels, so a result under a grouping reads as `... Role Play _ <grouping name> > <tile name>`. Content and exact separator are both settled.
