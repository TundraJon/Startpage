...
- [ ] **Correction — the user caught a real gap in my first pass at this reconciliation:** I'd claimed "profile icon opens Settings, unchanged, no new work" was already fully satisfied. That's only half true. `#profile-btn` (index.html/script.js) is a static icon button wired to `openSettings` — real, built. But the document's actual Section 2 content (tap the icon → **native photo picker** → the browser holds a **local reference to a photo already on the user's device**, nothing uploaded, so the icon shows their own picture) **does not exist at all** — no `<input type="file">`, no stored image reference, nothing. The icon is just a fixed profile glyph today. **This is real, not-yet-built work**, not something superseded by anything already shipped — it needs its own design pass (where's the image stored — `localStorage` can't hold a large photo efficiently, so this likely wants IndexedDB or an object URL re-picked each session; is there a default/placeholder state when nothing's been chosen; does tapping the already-set icon re-open the picker to change it, vs. needing a separate settings entry) before it's buildable. Logged here, not the Build Queue, since none of that is decided yet.
- [ ] **Second pass, per the user's explicit request to re-check the document — two more findings:**
  - **Settings is still a popup, not a full-screen view — a real, separate gap, decided in favor of the document.** Section 3 wants Settings built as its own dedicated full/near-full-screen view (like the Reorg Tree Tool). `#settings-overlay` (index.html) is currently just the same dimmed-backdrop `.help-overlay` popup pattern every other dialog uses — confirmed by reading the CSS, not assumed. The document's original reasoning ("given how much lives inside it, particularly Colors") is largely moot now that Colors isn't centralizing into Settings, but asked separately anyway since it's a distinct architectural claim — **the user confirmed they still want it: Settings should be converted to a dedicated full-screen view**, independent of the Colors decision. Not yet built — needs its own design pass (does it get the Reorg Tool's exact treatment — a separate `#settings-view` full-page div instead of an overlay, Cancel/Save or immediate-apply semantics for its existing Site Name/Theme/Weather/Organize sections, etc.).
  - **Grouping label text color — a real gap, but out of *this* document's own scope, not decided here.** Section 4.3 assumes a color picker for grouping labels (the Free/Paid-style divider text) already exists, cross-referencing a *different*, not-currently-provided document ("Visual Grouping Headers, Section 4"). Confirmed by reading the code: no such picker exists anywhere — `.tile-divider-label` just renders in a fixed muted color (`var(--fg-muted)`), no stored color field, no UI for it. This document explicitly scopes grouping-label color OUT of the Colors sub-screen either way, so it isn't this document's call to make — just flagged here as a known related gap, in case that other document (or a future instruction) brings it back into play.
