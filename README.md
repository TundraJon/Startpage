...
**Test/preview mode:** a new **"Preview Condition Skins"** entry in Weather Options' existing "Visual Flourishes" section, opening a picker with all 10 states below. Multi-select (needed for compound conditions like Thunderstorm + Hail). Selected state(s) override the live skin regardless of actual conditions, clearly marked as preview (small "PREVIEW" label/border), exit via tap-away/close/"Return to Live". **Coordination note:** this is a new *permanent* UI element, distinct from the existing *temporary* `🧪 Testing Panel` (marked for eventual removal) that already covers time-of-day and cloud % — the two will coexist for a while; the new picker doesn't replace the testing panel's existing controls.
